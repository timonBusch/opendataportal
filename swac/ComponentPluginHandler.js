...
class ComponentPluginHandler {
    ...
    constructor(requestor) {
        ...
        if (requestor.swac_comp.options.plugins) {
            ...
            SWAC_debug.addHintMessage('ComponentPluginHandler',
                    'The component >' + requestor.swac_comp.componentName
                    + '< supports plugins. You can set plugin options by adding the attribute plugins = new Map() to the requestors options.', requestor);
        } else {
            SWAC_debug.addDebugMessage('ComponentPluginHandler', 'There are no plugins configured.', requestor);
    ...
    getPlugin(pluginname) {
        ...
        if (!plugin) {
            ...
            SWAC_debug.addErrorMessage('ComponentPluginHandler', errMsg, this.requestor);
            throw(errMsg);
        }
        return plugin;
    }
    ...
    deactivatePlugin(pluginname) {
        let plugin = this.plugins.get(pluginname);
        if (!plugin) {
            SWAC_debug.addErrorMessage('ComponentPluginHandler', 'Plugin >'
            ...
            if (pluginname) {
                ...
                this.loadPlugin(pluginname).then(function (loadResult) {
                    ...
                    thisComponentPluginHandler.loadPlugins(pluginname).then(function (loadedSubPlugins) {
                        ...
                        SWAC_debug.addDebugMessage(requestor.swac_comp.componentName, 'Plugins loading done', requestor);
                        // Create the event
                    ...
                    SWAC_debug.addErrorMessage('ComponentPluginHandler', 'Could not load plugin >' + pluginname + '>: ' + error);
                    reject();
                });
            } else {
                resolve(loadedPlugins);
    ...
    loadPlugin(pluginname) {
        return new Promise((resolve, reject) => {
            ...
            if (this.loadedPlugins.includes(pluginname)) {
                SWAC_debug.addErrorMessage('ComponentPluginHandler', 'Plugin >' + pluginname + '< is allready loaded.');
                resolve();
                return;
            }
            ...
            if (!pluginconfig.active) {
                SWAC_debug.addDebugMessage('ComponentPluginHandler', 'Plugin >'
            ...
            if (pluginconfig.isApplicableFor) {
                ...
                if (this.requestor.swac_comp.data) {
                    ...
                    SWAC_debug.addErrorMessage('ComponentPluginHandler',
            ...
            let compShortName = this.requestor.swac_comp.componentName.replace('SWAC_', '');
            ...
            pluginrequestor.swac_comp.componentName = compShortName + '/plugins/' + pluginname;
            ...
            SWAC_language.loadComponentTranslation(pluginrequestor).then(function () {
                ...
                thisComponentPluginHandler.loadPluginObject(pluginname).then(function (loadPluginObjectResult) {
                    thisComponentPluginHandler.loadPluginTemplate(pluginname).then(function (loadTemplateResult) {
                        thisComponentPluginHandler.initPluginObject(pluginname).then(function (initObjectResult) {
                            ...
                            SWAC_debug.addErrorMessage('ComponentPluginHandler', errMsg, thisComponentPluginHandler.requestor);
                            reject(errMsg);
                        });
                    }).catch(function (error) {
                ...
                SWAC_debug.addErrorMessage('ComponentPluginHandler', 'Could not load language for plugin: ' + error);
            });
        });
    }
    ...
    loadPluginTemplate(pluginname) {
        return new Promise((resolve, reject) => {
            ...
            if (plugNavTempls === null) {
                SWAC_debug.addDebugMessage('ComponentPluginHandler', 'There was no navigation area '
                ...
                for (let plugNavTempl of plugNavTempls) {
                    ...
                    if (!pluginnavAElem) {
                        SWAC_debug.addErrorMessage('ComponentPluginHandler', 'The components template must contain a a-element within the swac_repeatedForPluginNav.', this.requestor);
                        return;
                    }
            ...
            if (plugContAreas === null) {
                SWAC_debug.addDebugMessage('ComponentPluginHandler', 'There was no area template '
            ...
            if (template && template.style) {
                SWAC_debug.addDebugMessage('view', 'Load CSS >' + template.style + '< for >' + plugin.componentName + '<');
...
//                                thisComponentPluginHandler.plugins.set(pluginname, plugin);
                            ...
                            let errMsg = 'Script for plugin >' + pluginname
                                    + '< was loaded. But there is no >' + pluginname + 'Factory< in it.';
                            SWAC_debug.addErrorMessage('ComponentPluginHandler', errMsg,
    ...
    initPluginObject(pluginname) {
        return new Promise((resolve, reject) => {
            ...
            if (typeof plugin.init === 'undefined') {
                SWAC_debug.addDebugMessage('ComponentPluginHandler', 'Plugin >' + pluginname + '< has no init() function.', this.requestor);
                resolve(plugin);
                return;
            }
