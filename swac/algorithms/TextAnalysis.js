...
class TextAnalysis {
    ...
    init() {
        return new Promise((resolve, reject) => {
            let thisRef = this;
            fetch(SWAC_config.swac_root + '/swac/data/dictionary/' + lang + '.json').then(
                    function (response) {
                        response.json().then(function (json) {
                            thisRef.dictionary = json;
                            SWAC_debug.addDebugMessage('TextAnalysis', 'Loaded dictionary >' + lang + '<');
                            if (thisRef.abbrivations)
                                resolve();
                        }).catch(
                                function (error) {
                                    SWAC_debug.addErrorMessage('TextAnalysis', 'Could not parse dictionary: ' + error);
                                    reject();
                                });
                    }
            ).catch(
                    function (error) {
                        SWAC_debug.addErrorMessage('TextAnalysis', 'Could not load dictionary for >' + lang + '<');
                        reject();
                    }
            );
            // Load abbrivations
            fetch(SWAC_config.swac_root + '/swac/data/abbrivations/' + lang + '.json').then(
                    function (response) {
                        response.json().then(function (json) {
    ...
    getAbbrivationResolved() {
        ...
        if (this.usedabbrs.length > 0) {
            SWAC_debug.addDebugMessage('TextAnalysis', 'Text uses >' + this.usedabbrs.length + '< abbrivations.');
            SWAC_debug.addDebugMessage('TextAnalysis', 'Abbrivation resolved text: >' + this.text_abbrfree + '<');
        }
