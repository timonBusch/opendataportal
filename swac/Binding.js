...
class Binding {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!this.component.data || (Object.entries(this.component.data).length === 0 && this.component.data.constructor === Object)) {
                if (this.component.options !== 'undefined'
                    ...
                    SWAC_debug.addDebugMessage('Binding', 'There is no data so this '
                ...
                if (this.requestor.fromName) {
                    SWAC_debug.addDebugMessage('Binding', "There is no data with name >"
    ...
    bindData(data) {
        ...
        if (data[lastSource] && data[lastSource][0]) {
            ...
            for (let curAttr in bindPoints) {
                for (let curBindPoint of bindPoints[curAttr]) {
                    curBindPoint.dataset = data[lastSource][0];
                }
            }
        }
    }
    ...
    bindSet(set) {
        ...
        if (this.component.conditions) {
            for (let condOn in this.component.conditions) {
                ...
                if (typeof set[condOn] === 'undefined') {
                    SWAC_debug.addErrorMessage('Binding', 'Dataset >' + set.swac_fromName + "["
                            + set.swac_setNo + "]< does not contain the expected attribute >"
                            + condOn + "<. Condition check failed.");
                    if (SWAC_config.debugmode) {
                        // Look if there is one data named similar
                        for (let curAttrName in set) {
                            if (curAttrName.indexOf(condOn) > -1 || condOn.indexOf(curAttrName) > -1) {
                                SWAC_debug.addHintMessage('Binding', "Maybe you have an typo and want to use attribute >" + curAttrName + "<");
                            }
                        }
                    }
                }
