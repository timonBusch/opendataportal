...
SWAC_language.init = function () {
    return new Promise((resolve, reject) => {
        ...
        if (typeof SWAC_config.lang !== 'undefined') {
            ...
            SWAC_debug.addDebugMessage('swac', 'There is no language configured in configuration. Automatic choosen >' + this.activeLang + '< from browser.');
        }
        // Core language file
        ...
        this.loadTranslationFile(langfile_url, objectname).then(function () {
            ...
            SWAC_debug.addErrorMessage('language',
...
SWAC_language.loadComponentTranslation = function (requestor) {
    return new Promise((resolve, reject) => {
        ...
        let objectname = requestor.swac_comp.componentName;
        ...
        SWAC_language.loadTranslationFile(langfile_url, objectname).then(function () {
            ...
            SWAC_debug.addErrorMessage('language', 'Could not load language file for >'
                    + this.activeLang + '< for component >'
                    + requestor.swac_comp.componentName + '<: ' + error);
            reject();
        });
    });
};
...
SWAC_language.loadTranslationFile = function (filepath, objectname) {
    return new Promise((resolve, reject) => {
        ...
        if (this.loadedLangs.includes(filepath)) {
            SWAC_debug.addDebugMessage('Language','Language file >' + filepath + '< is allready loaded.');
            resolve();
            return;
        }
        // Check if translation is loading
        if (this.loadingLangs.has(filepath)) {
            SWAC_debug.addDebugMessage('Language','Waiting for language file >' + filepath + '< to be loaded.');
            ...
            SWAC_debug.addDebugMessage('Language','Loading language file >' + filepath + '<');
