var ScreenboardFactory = {};
ScreenboardFactory.create = function (config) {
    return new Screenboard(config);
};

/* 
 * This component realises an onscrean input board for tablet devices
 */
class Screenboard extends Component {

    constructor(options = {}) {
        super(options);
        this.componentName = 'Screenboard';
        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.options.for.length > 0) {
                for (let i in this.options.for) {
                    let inputField = document.querySelector(this.options.for[i]);
                    inputField.addEventListener('focus', this.onFocus.bind(this));
                }
            } else {
    ...
    onFocus(evt) {
        ...
        if (this.touchmode || this.options.onlyOnTouch === false) {
            ...
            SWAC_debug.addDebugMessage('Screenboard',
    ...
    onKeyTipp(evt) {
        ...
        if (!specialPressed && val.indexOf('icon:') > -1) {
            SWAC_debug.addDebugMessage('Screenboard', 'Pressed a button with icon but no special handling.');
            UIkit.notification({
                message: SWAC_language.Screenboard.nofunction,
                status: 'error',
