...
class Guid extends Component {
            ...
            for (let curSource in this.data) {
                ...
                for (let evt of this.data[curSource]) {
                    ...
                    for (let elem of elems) {
                        ...
                        // Add event listener
                        SWAC_debug.addDebugMessage('Guide', 'Added listener for'
    ...
    handle(evt) {
        ...
        SWAC_debug.addDebugMessage('Guide', 'Now executeing step >'
        ...
        if (!appEvt) {
            ...
            if (!appEvt) {
                SWAC_debug.addDebugMessage('Guide', 'Found no next step ending presentation.', this.requestor);
                appEvt = this.endstep;
            }
        }
        ...
        if (appEvt.onevt !== 'step' && appEvt.onevt !== evt.type) {
            // Do not execute step
            SWAC_debug.addDebugMessage('Guide','Do not execute step because event is not matching.', this.requestor);
            this.guidestates.step--;
            return;
            ...
            if (!found) {
                SWAC_debug.addDebugMessage('Guide','Do not execute step because '
