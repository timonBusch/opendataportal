var BookingFactory = {};
BookingFactory.create = function (config) {
...
only editing the form template. All entered data will be send automatically to the backend.'
    ...
    init() {
        return new Promise((resolve, reject) => {
            ...
            this.includeBookings().then(
                        ...
                        for (let curItemElem of itemElems) {
                            ...
                            if (!notAvElem) {
                                ...
                                if (avtxtElem.innerHTML === '')
                                    avtxtElem.innerHTML = SWAC_language.Booking.available;
                ...
                SWAC_debug.addDebugMessage('Booking',
    ...
    includeBookings() {
        return new Promise((resolve, reject) => {
            ...
            if (curBookingsRequestor) {
                let thisRef = this;
                SWAC_model.load(curBookingsRequestor).then(
                        function (response) {
                            ...
                            for (let [itemid, bookings] of thisRef.bookings) {
                                ...
                                if (itemElem) {
                                    let avail = true;
                                    // Get booked item
                                    let item = thisRef.data[firstSource][itemid];
                                    ...
                                    if (!avail) {
                                        ...
                                        avtxtElem.innerHTML = SWAC_language.Booking.notavailable;
                                        // Get booking button
                                        ...
                                        if (reavailFrom) {
                                            ...
                                            let infoTxt = document.createTextNode(SWAC_language.Booking.availability_again_from + ' ');
                                        ...
                                        let avtxt = SWAC_language.Booking.placesleft;
                                        avtxt = SWAC_language.replacePlaceholders(avtxt, 'avail', availplaces);
                                        avtxt = SWAC_language.replacePlaceholders(avtxt, 'total', item.places);
                                        avtxtElem.innerHTML = avtxt;
                                    }
                                } else {
                                    SWAC_debug.addErrorMessage('Booking',
    ...
    onClickOpenBooking(evt) {
        if (!this.options.bookRequestor) {
            SWAC_debug.addDebugMessage('Booking', 'bookRequestor is not set, booking over form is not available.', this.requestor);
            UIkit.modal.alert(SWAC_language.Booking.bookingpermail);
            return;
        }
        ...
        let repForSet = this.requestor.swac_view.findReapeatedForSet(evt.target);
    ...
    onClickSubmitBooking(evt) {
        ...
        // Add metadata
        dataCapsle.metadata = {};
        dataCapsle.metadata.fromSource = this.options.bookRequestor.fromName;
        dataCapsle.metadata.fromWheres = this.options.bookRequestor.fromWheres;
        // Add data
        dataCapsle.data = [];
        dataCapsle.data[0] = {};
        ...
        SWAC_model.save(dataCapsle).then(function (json) {
            UIkit.modal.alert(SWAC_language.Booking.booksucseed);
        }).catch(function (error) {
            UIkit.modal.alert(SWAC_language.Booking.bookerror);
        });
    }
}
