var mediatagsFactory = {};
mediatagsFactory.create = function (pluginconfig) {
    return new MediatagsSPL(pluginconfig);
};

class MediatagsSPL extends ComponentPlugin {

    constructor(options = {}) {
        super(options);
        this.componentName = 'Mediaeditor/plugins/mediatags';
        this.desc.depends[0] = {
            name: 'Line.js',
            path: SWAC_config.swac_root + '/swac/components/Mediaeditor/plugins/mediatags/Line.js',
            desc: 'Represents a line of a polygon.'
        };
        this.desc.depends[1] = {
            name: 'LineFunction.js',
            path: SWAC_config.swac_root + '/swac/components/Mediaeditor/plugins/mediatags/LineFunction.js',
            desc: 'Represents a line as a function f(x)=mx+b'
        };
        this.desc.depends[2] = {
            name: 'LineSegment.js',
            path: SWAC_config.swac_root + '/swac/components/Mediaeditor/plugins/mediatags/LineSegment.js',
            desc: 'Represents a linesegment between two points'
        };
        this.desc.depends[3] = {
            name: 'Polygon.js',
            path: SWAC_config.swac_root + '/swac/components/Mediaeditor/plugins/mediatags/Polygon.js',
            desc: 'Represents a Polygon as set of vertices.'
        };
        this.desc.depends[4] = {
            name: 'Vertex.js',
            path: SWAC_config.swac_root + '/swac/components/Mediaeditor/plugins/mediatags/Vertex.js',
    ...
    createPossibleTagtypesList() {
        if (!this.options.tagtypesrequestor) {
            SWAC_debug.addErrorMessage('MediatagsSPL', 'The option >tagtypesrequestor< is not set. You must specify a requestor where the plugin can get informations about possible tagtypes. Otherwise no new tags can be created.');
            return;
        }

        // Get addTag-Buttons
        let addTagButtons = this.requestor.querySelectorAll('.swac_mediatags_add');
        let selectTypeElems = this.requestor.querySelectorAll('.swac_mediatags_typeselect');

        let thisRef = this;
        // Get available tags
        SWAC_model.load(this.options.tagtypesrequestor).then(function (model) {
            if (model.data.length < 1) {
                // Hide buttons
                for (let curAddTagButton of addTagButtons) {
                    curAddTagButton.classList.add('swac_dontdisplay');
                    let msg = document.createTextNode(SWAC_language.Mediaeditor.mediatags.noadding);
            ...
            SWAC_debug.addErrorMessage('MediatagsSPL', 'Could not create tagtype selection box: ' + error);
            // Hide buttons
            for (let curAddTagButton of addTagButtons) {
                curAddTagButton.classList.add('swac_dontdisplay');
                let msg = document.createTextNode(SWAC_language.Mediaeditor.mediatags.noaddingfailure);
                curAddTagButton.parentNode.appendChild(msg);
            }
        });
    }
    ...
    createExistingTags(fromName, set) {
        if (!this.options.tagsrequestor) {
            ...
            SWAC_debug.addErrorMessage('MediatagsSPL', msg);
            throw(msg);
        }
        ...
        let thisRef = this;
        SWAC_model.load(requestor).then(function (models) {
            // Save existing tags
            ...
            if (models.data.length === 0) {
                ...
                for (let curSetElem of setElems) {
                    ...
                    if (tagsElem) {
                        ...
                        repeatedElem.innerHTML = SWAC_language.Mediaeditor.mediatags.notags;
            ...
            SWAC_debug.addErrorMessage('MediatagsSPL',
            ...
            for (let curSetElem of setElems) {
                ...
                if (tagsElem) {
                    ...
                    repeatedElem.innerHTML = SWAC_language.Mediaeditor.mediatags.notagsfailure;
                    repeatForElem.parentNode.appendChild(repeatedElem);
                }
            }
        });
    ...
    createMediaTagLayer(mediaTag, fromName, set) {
        if (mediaTag.polygons) {
            if (typeof mediaTag.polygons === 'string'
                ...
                SWAC_debug.addDebugMessage('MediatagSPL', 'mediaTag.polygons are deliverd as string. Please use json array instead.');
            }

            // Search the area containing the media
            let mediaContArea = this.requestor.swac_comp.getMediaContArea(fromName, set.id);
            // Get scaleElem
            let scaleElem = mediaContArea.querySelector('[scale]');
            let scale = scaleElem.getAttribute('scale');
            // Create drawing layer
            let drawlayer = this.requestor.swac_comp.createDrawingLayer(fromName, set.id);
            ...
            for (let curPoly of mediaTag.polygons) {
                ...
                if (curPoly.vertices) {
                    ...
                    try {
                        ...
                        SWAC_debug.addErrorMessage('MediatagsSPL', 'Could not draw polygon: ' + error);
                    }
                    drawlayer.swac_polygons.push(poly);
                } else {
                    SWAC_debug.addDebugMessage('MediatagsSPL', 'A polygon does not contain vertices.');
                }
            }
            return drawlayer;
        }
    ...
    createMediaTagListEntry(mediaTag, mediaSet, layer) {
        // Remove no sets info
        let noSetsElem = this.requestor.querySelector('.swac_noSets');
        if (noSetsElem)
            noSetsElem.parentNode.removeChild(noSetsElem);
        // Find all areas for the current set and search the taglist within
        let setElems = this.requestor.querySelectorAll('[swac_setid="' + mediaSet.id + '"]');
        for (let curSetElem of setElems) {
            let tagsElem = curSetElem.querySelector('.swac_mediatags_list');
            if (tagsElem) {
                let repeatForElem = tagsElem.querySelector('.swac_repeatForSet');
                ...
                if (!mediaTag.id) {
                    ...
                    warnElem.setAttribute('uk-tooltip', SWAC_language.Mediaeditor.mediatags.notsavedtag);
                }
    ...
    onDeleteMediaTag(evt) {
        ...
        if (!setid || setid === 'undefined') {
            ...
            let deleterequestor = {
                ...
                metadata: {
                    fromSource: this.options.tagsrequestor.fromName
                }
            };

            SWAC_model.delete(deleterequestor).then(
                    function (response) {
                        // Remove layer and list element
                        searchElem.swac_layer.parentNode.removeChild(searchElem.swac_layer);
                        searchElem.parentNode.removeChild(searchElem);
    ...
    onEditMediaTag(evt) {
        ...
        // Get media area
        let tagFromName = SWAC_model.getSetnameFromRefernece(tag.media);
        let mediaAreas = this.requestor.querySelectorAll('[swac_setname="' + tagFromName + '"][swac_setid="' + tag.id + '"]');
        for (let curMediaArea of mediaAreas) {
            ...
            if (wizzElem) {
                ...
                let tagtypeid = SWAC_model.getIdFromReference(tag.tagtype);
...
//******************************
    ...
    startCreateTag(evt, layer) {
        ...
        // Get set area
        let setarea = this.requestor.swac_view.findReapeatedForSet(pluginarea);
        let fromName = setarea.getAttribute('swac_setname');
        ...
        if (!layer) {
            // Create draw layer
            wizzArea.drawingLayer = this.requestor.swac_comp.createDrawingLayer(fromName, setid);
        } else {
    ...
    chooseTagtype(evt) {
        ...
        if (!wizzardArea.drawingLayer.swac_polygons
            ...
            UIkit.modal.confirm(SWAC_language.Mediaeditor.mediatags.nopolygoncreated, {stack: true}).then(function () {
                return;
            }, function () {
                return;
            });
    ...
    chooseTitle(evt) {
        ...
        if (parseInt(value) === 0) {
            UIkit.modal.confirm(SWAC_language.Mediaeditor.mediatags.notagtypeselected, {stack: true}).then(function () {
                return;
            }, function () {
                return;
            });
    ...
    addChooseDesc(evt) {
        ...
        nextButton.addEventListener('click', nextButton.fn);
        nextButton.innerHTML = SWAC_language.Mediaeditor.mediatags.savetag;
    }
    ...
    saveTag(evt) {
        ...
        // Rename next button
        let nextButton = pluginarea.querySelector('.swac_mediatags_next');
        nextButton.innerHTML = SWAC_language.Mediaeditor.mediatags.wizzardnext;
        nextButton.removeEventListener('click', nextButton.fn);
        // Hide wizzard form
        let wizzArea = pluginarea.querySelector('.swac_mediatags_wizzard');
        ...
        for (let curPoly of wizzArea.drawingLayer.swac_polygons) {
            ...
            for (let curVertex of curPoly.vertices) {
                let pictureVertex = this.requestor.swac_comp.convertDrawPointToMediaPoint(curVertex, wizzArea.drawingLayer);
        ...
        let thisRef = this;
        SWAC_model.save(saverequestor).then(function (result) {
...
//*****************************
    ...
    drawStart(evt) {
        ...
        SWAC_debug.addDebugMessage('MediatagsSPL', 'Start drag drawing.');

        // Create vertex for actual mouse position
        let mousepos = this.requestor.swac_comp.getMousePositionOnMedia(evt);
    ...
    drawMove(evt) {
        if (evt.target.swac_drawmode === 'drag') {
            ...
            this.requestor.swac_comp.clearCanvasLayer(evt.target);
            let polys = evt.target.swac_polygons;
    ...
    drawStop(evt) {
        ...
        SWAC_debug.addDebugMessage('MediatagsSPL', 'Stop drag drawing.');
    ...
    onDrawStopOutOfLayer(evt) {
        ...
        if (this.currentDrawStartElem) {
            ...
            if (!mousepos) {
                ...
                // Clear previous drawing
                this.requestor.swac_comp.clearCanvasLayer(this.currentDrawStartElem);
                // Draw polygons
