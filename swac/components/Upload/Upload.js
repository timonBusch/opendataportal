    ...
    init() {
        return new Promise((resolve, reject) => {
            if (!this.options.uploadTargetURL) {
                SWAC_debug.addErrorMessage('upload',
                ...
                beforeAll: function (p) {
                    for (let curFile of arguments[1]) {
                        ...
                        try {
                            ...
                            SWAC_debug.addErrorMessage('Upload', 'Error before uploading: ' + error);
                            UIkit.notification({
...
//                    console.log(p);
                ...
                error: function (error) {
                    SWAC_debug.addErrorMessage('Upload', 'Error while uploading: ' + error, thisRef.requestor);
                }
            });
    ...
    generateStatusTR(upFile) {
        SWAC_debug.addDebugMessage('upload', 'Generate statusTR for uploadid: ' + upFile.uploadid);
    ...
    getSendAlongData() {
        ...
        for (let curComp of this.options.dataComponents) {
            SWAC_debug.addDebugMessage('Upload','Getting sendAlongData from >' + curComp.selector + '<');
            ...
            if (curCompElem.hasAttribute("swa")) {
                ...
                if(curCompElem.name === 'name') {
                    SWAC_debug.addDebugMessage('Upload','You used an imput field >name< for sendAlongData. This will override the filename.');
                }
    ...
    generateProcessingResultView(processedfile) {
        ...
        SWAC_debug.addDebugMessage('upload', 'Create processingResultView for uploadid: ' + uploadid);
    ...
    generateProessingIcon(uploadid) {
        SWAC_debug.addDebugMessage('upload', 'Generate processingIcon for uploadid: ' + uploadid);
        let statusTR = document.getElementById('status_' + uploadid);
    ...
    generateProcessedIcon(uploadid) {
        SWAC_debug.addDebugMessage('upload', 'Generate processedIcon for uploadid: ' + uploadid);
        let statusTR = document.getElementById('status_' + uploadid);
        let statusTD = statusTR.querySelector('.statustd');
