...
class Iconized extends Diagram {
    ...
    drawValueDiagram(value) {
        SWAC_debug.addErrorMessage('Iconized', 'The Iconized Diagram supports does not support single values.');
    }
    ...
    drawSetDiagram(set) {
                    ...
                    if (set[parts[0]]) {
                        ...
                        switch (parts[1]) {
                            ...
                            case '>':
                                ...
                                break;
                            default:
                                SWAC_debug.addDebugMessage('Iconized', 'There is no test method for >' + parts[1] + '<');
                        }
                    }
                }
            }
        ...
        if (!bestMatching.curIcon) {
            ...
            SWAC_debug.addDebugMessage('Iconized', 'Found no icon for set >' + set.id + '<');
        } else {
