var IconFactory = {};
IconFactory.create = function (config) {
    return new Icon(config);
};

/**
 * Creating icons from data
 */
class Icon extends Component {
    ...
    init() {
        return new Promise((resolve, reject) => {
            ...
            for (let curSource in this.data) {
                // Go trough each dataset and search for iconable information
    ...
    afterAddDataset(fromName, set) {
        for (let attr in set) {
            ...
            if (this.options.showAttributes.length === 0
                ...
                this.requestor.appendChild(iconElem);
            }
        }
    }
    ...
    getIconElement(iconSearchExpression) {
        ...
        if (icoFilePath.endsWith('unknown.svg')) {
            let msg = SWAC_language.Icon.noIconFor.replace('%iconSearchExpression%', iconSearchExpression);
    ...
    getIconFilePath(iconSearchExpression) {
        ...
        let icoFilePath = SWAC_config.swac_root + '/swac/components/Icon/imgs/' + icoFileName;
        return icoFilePath;
    }
    ...
    getIconFileName(iconSearchExpression) {
        ...
        if (!iconFileName) {
            ...
            if (!iconFileName) {
                SWAC_debug.addErrorMessage('icon', 'There is no icon for >' + iconSearchExpression + '<');
