    ...
    init() {
        return new Promise((resolve, reject) => {
            ...
            for (let assetid of this.options.ionassets) {
                SWAC_debug.addDebugMessage('Worldmap', 'Loading CesiumIon asset >' + assetid + '<', this.requestor);
            ...
            SWAC_debug.addDebugMessage('Worldmap', 'running on Cesium ' + Cesium.VERSION, this.requestor);
            ...
            if (hid !== null) {
                ...
                setTimeout(function () {
                    ...
                    modelPromise.then(function (searchresult) {
                        ...
                        SWAC_debug.addErrorMessage('cesium', 'Could not load model: ' + error);
...
//console.log("TESTTHERE!");
            ...
            modelFactory.loadModel(datasource.url, this.requestor, datasource).then(
                    function (model) {
                        ...
                        model.draw(thisRef.viewer).then(function (entityref) {
                            ...
                            SWAC_debug.addErrorMessage('cesium', 'Error drawing model >'
                                    + datasource.url + '<: ' + error);
                        });
                    }
            ).catch(function (error) {
                SWAC_debug.addErrorMessage('cesium', 'Error loading model >'
                        + datasource.url + '<: ' + error);
            });
        }
    }
    ...
    findAndDrawModelsInViewport() {
        ...
        this.findModelsInViewport().then(function (models) {
                        ...
                        if (!thisRef.failedmodels.includes(modelurl)) {
                            modelFactory.loadModel(modelurl, thisRef.requestor, viewportdef).then(function (model) {
                                ...
                                model.draw(thisRef.viewer).then(function () {
                                    ...
                                    SWAC_debug.addErrorMessage('cesium', error);
                                });
                            }).catch(function (error) {
            ...
            SWAC_debug.addErrorMessage('cesium', 'Error while searching models in viewport: ' + error, this.requestor);
        });
    }
                ...
                if (distance <= viewportapi.below && !viewportapi.unreachable) {
                    ...
                    fetch(apiurl).then(function (response) {
                        if (response.status === 404) {
                            SWAC_debug.addErrorMessage('cesium', 'viewport api >'
                            ...
                            response.json().then(function (data) {
                                ...
                                returnedApis++;
                                SWAC_debug.addErrorMessage('cesium', 'Could not parse data from viewport model request: ' + error);
                                if (returnedApis === this.options.model_zoomlevels.length) {
                                    resolve(foundModels);
                                }
                            });
                        ...
                        returnedApis++;
                        SWAC_debug.addErrorMessage('cesium', 'Could not get models for viewport from >' + apiurl + '<: ' + error);
                        if (returnedApis === this.options.model_zoomlevels.length) {
                            resolve(foundModels);
                        }
                    });
...
//    console.log(Cesium.JulianDate.equalsEpsilon(this.timeddata.lastrendered, currentTime,1));
        ...
        if (this.timeddata.lastrendered === null
            ...
            SWAC_debug.addDebugMessage('cesium', 'Time changed to: ' + currentTime);
