...
class SEGLTF extends Model {
        ...
        return new Promise((resolve, reject) => {
            this.preLoad(filepath).then(function (data) {
                ...
                if (typeof data === 'undefined') {
                    ...
                    SWAC_storage.files[modelobj._filepath].data = Cesium.Model.fromGltf(gltfRequestor);

                    SWAC_debug.addDebugMessage('SEGLTF', 'Model >'
                            + modelobj._filepath + '< succsessfull loaded.');

                    // Get url of groundjson
                    let geojsonurl = SWAC_storage.files[modelobj._filepath].url.replace('.glb', '.geojson');

                    modelFactory.loadModel(geojsonurl).then(function (geojson) {
                        SWAC_debug.addDebugMessage('SEGLTF', 'Loaded geojson with metadata for ' + filepath);
                        modelobj._locations = geojson.locations;
                        modelobj._geojson = geojson;
                        resolve();
                    }).catch(function (error) {
                        SWAC_debug.addErrorMessage('SEGLTF', 'Could not load metadata for ' + filepath + ' error: ' + error);
                        resolve();
                    });
                }
            }).catch(function (error) {
    ...
    draw(viewer) {
        ...
        return new Promise((resolve, reject) => {
            ...
            // Add model to globe
            let modelReference = viewer.scene.primitives.add(SWAC_storage.files[this._filepath].data);
            thisModel._drawnref = modelReference;
            // Wait for model to be ready
            modelReference.readyPromise.then(function (loadedmodel) {
                SWAC_debug.addDebugMessage('SEGLTF', 'Model >' + loadedmodel._resource._url + '< succsessfull drawn.');
            ...
            modelReference.readyPromise.otherwise(function (error) {
                SWAC_debug.addErrorMessage('SEGLTF', 'The following error occured while trying to render the model: ' + error);
