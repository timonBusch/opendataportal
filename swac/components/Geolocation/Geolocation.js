...
class Geolocation extends Component {
    ...
    constructor(options) {
        super(options);
        this.componentName = 'Geolocation';
    ...
    init() {
        ...
        if (!navigator.geolocation) {
            SWAC_debug.addDebugMessage('geolocation', 'The browser does not support geolocation. Hiding all elements with class .swac_geolocation');
            this.nolocate();
        }
    ...
    onError(error) {
        ...
        switch (error.code) {
            case error.PERMISSION_DENIED:
                SWAC_debug.addDebugMessage('geolocation', 'User denied the request for Geolocation.');
                let geoElems = document.querySelectorAll(".swac_geolocation");
                for (let geoElem of geoElems) {
                    geoElem.style.display = "none";
                }
            ...
            case error.POSITION_UNAVAILABLE:
                SWAC_debug.addDebugMessage('geolocation', "Location information is unavailable.");
                infoElem.innerHTML = SWAC_language.Geolocation.unavailable;
                console.log(infoElem);
                break;
            case error.TIMEOUT:
                SWAC_debug.addDebugMessage('geolocation', "The request to get user location timed out.");
                infoElem.innerHTML = SWAC_language.Geolocation.timeout;
                break;
            case error.UNKNOWN_ERROR:
                SWAC_debug.addDebugMessage('geolocation', 'An unknown error occurred.');
