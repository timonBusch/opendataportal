...
class ComponentHandler {
    ...
    loadComponent(requestor) {
        if (!requestor.id) {
            SWAC_debug.addErrorMessage('ComponentHandler', 'All requestor DOMElement must have the id attribute.', requestor);
            return;
        }
        SWAC_debug.addDebugMessage('ComponentHandler', 'Loading component for requestor >' + requestor.id + '<');
                        ...
                        thisRef.loadComponentData(requestor).then(function () {
                            ...
                            binding.bind().then(function () {
                                thisRef.initComponent(requestor).then(function (requestor) {
                                    ...
                                    if (error)
                                        SWAC_debug.addErrorMessage('ComponentHandler', 'Could not init component for >' + requestor.id + '<: ' + error);
                                    view.removeLoadingElem(requestor);
                                });
                            }).catch(function (error) {
                                if (error)
                                    SWAC_debug.addErrorMessage('ComponentHandler', 'Could not bind for >' + requestor.id + '<: ' + error);
                                view.removeLoadingElem(requestor);
                            });
                        }).catch(function (error) {
                            if (error)
                                SWAC_debug.addErrorMessage('ComponentHandler', 'Could not load data for >' + requestor.id + '<: ' + error);
                            view.removeLoadingElem(requestor);
                        });
                    }).catch(function (error) {
                        if (error)
                            SWAC_debug.addErrorMessage('ComponentHandler', 'Could not load template for >' + requestor.id + '<: ' + error);
                        view.removeLoadingElem(requestor);
                    });
                }).catch(function (error) {
                    if (error)
                        SWAC_debug.addErrorMessage('ComponentHandler', 'Could not load translation for >' + requestor.id + '<: ' + error);
                });
            }).catch(function (error) {
                if (error)
                    SWAC_debug.addErrorMessage('ComponentHandler', 'Could not load object for >' + requestor.id + '<: ' + error);
            });
        }).catch(function (error) {
            if (error)
                SWAC_debug.addErrorMessage('ComponentHandler', 'Could not load options for >' + requestor.id + '<: ' + error);
        });
    ...
    parseDeclaration(requestor) {
        ...
        requestor.swac_comp = {};
        requestor.swac_comp.componentName = component_config.split(" ")[0];
                        ...
                        for (let wherestr of wheres) {
                            ...
                            if (varmatches !== null && varmatches.length > 0) {
                                for (let i in varmatches) {
                                    ...
                                    if (param === null) {
                                        SWAC_debug.addDebugMessage('swac', 'Parameter >'
                            ...
                            if (whereval !== null) {
                                ...
                                SWAC_debug.addDebugMessage('swac', 'Parameter >' + wherekey + '< for >' + requestor.id + '< has null value.', requestor);
                            }
                        }
                        break;
                    case 'OPTIONS':
     ...
     * 
     * @param {DOMElement} requestor DOM Element requesting the component
     * @returns {Promise}
     */
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!requestor.fromName) {
                if (requestor.swac_comp.options !== 'undefined'
                    ...
                    SWAC_debug.addDebugMessage('bind', 'There is no data requested so this '
                ...
                SWAC_debug.addDebugMessage('ComponentHandler', 'No data requested for >'
            ...
            SWAC_model.load(requestor).then(function (dataCapsle) {
                ...
                if (dataCapsle.metadata) {
                    ...
                    SWAC_debug.addErrorMessage('ComponentHandler', 'Data loading returned without metainformation.', requestor);
                    reject();
                }
    ...
    loadComponentObject(requestor) {
        return new Promise((resolve, reject) => {
            ...
            dependencyStack.push({path: SWAC_config.swac_root + '/swac/components/'
                        + requestor.swac_comp.componentName + '/'
                        + requestor.swac_comp.componentName + '.js'});
            SWAC.loadDependenciesStack(dependencyStack, requestor.swac_comp).then(function () {

                // Check if object is available
                if (typeof window[requestor.swac_comp.componentName + 'Factory'] !== 'undefined') {
                    // Create component to load dependencies
                    SWAC_language[requestor.swac_comp.componentName] = {};
                    requestor.swac_comp = window[requestor.swac_comp.componentName + 'Factory'].create(requestor.swac_comp.options);
                    requestor.swac_comp.requestor = requestor;
                    // Debug function
                    requestor.swac_comp.plugins.unloadPlugins = function () {
                        SWAC_debug.addErrorMessage(requestor.swac_comp.componentName, 'Cant access plugin function >unloadPlugins()< before initialisation has finished.');
                    };

                    // Generate options hint
                    SWAC_debug.addOptionsHintMessage(requestor);
                    ...
                    SWAC_debug.addErrorMessage('ComponentHandler',
                            'Factory for >' + requestor.swac_comp.componentName
                ...
                SWAC_debug.addErrorMessage('ComponentHandler',
                        'Could not load script for >' + requestor.swac_comp.componentName
                        + '<: ' + error, requestor);
                reject();
            });
        });
    ...
    initComponent(requestor) {
        return new Promise((resolve, reject) => {
            ...
            document.addEventListener('swac_' + requestor.id + '_complete', function (event) {
                SWAC_debug.addDebugMessage('ComponentHandler', 'swac_' + requestor.id + '_complete called');
                requestor.swac_view.removeLoadingElem(requestor);
            ...
            if (typeof requestor.swac_comp.init === 'function') {
                requestor.swac_comp.init(requestor).then(function () {
                    try {
                        ...
                        SWAC_debug.addErrorMessage('ComponentHandler', 'Error after init()' + error);
                    }
                    resolve(requestor);
//                }).catch(function (error) {
//                    //DEV-Note: To find the source for the error, comment this catch-block out
//                    SWAC_debug.addErrorMessage('ComponentHandler', "Component >" + requestor.swac_comp.componentName + "< could not be initialized. Error in init(): >" + error + '<', requestor);
//                    this.requestor.swac_view.removeLoadingElem(requestor);
//                    reject();
                });
            } else {
                SWAC_debug.addDebugMessage('ComponentHandler', "Component >" + requestor.swac_comp.componentName + "< has no initiliser.");
