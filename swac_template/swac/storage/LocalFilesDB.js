...
class LocalFilesDB {
    ...
    constructor(dbname) {
        if (!window.indexedDB) {
            SWAC_debug.addErrorMessage('UploadFilesDB', 'The browser does not support indexedDB cant store files.');
            return;
        }
        this.dbname = dbname;
    ...
    open() {
        ...
        return new Promise((resolve, reject) => {
            ...
            dbOpenReq.onerror = function (error) {
                SWAC_debug.addErrorMessage('LocalFilesDB',
            ...
            dbOpenReq.onupgradeneeded = function (event) {
                SWAC_debug.addDebugMessage('LocalFilesDB',
                        ...
                        file.toObject().then(
                                function (saveableFile) {
                                    ...
                                    request.onerror = function (error) {
                                        SWAC_debug.addErrorMessage('LocalFilesDB', 'Could not save file into database.');
                                    };
                                });
                    }
            );
    ...
    updateFile(file) {
        ...
        return new Promise((resolve, reject) => {
            if (!file.localdb_key) {
                SWAC_debug.addErrorMessage('LocalFilesDB', 'The given file does not contain the localdb_key attribute.');
                reject();
                return;
            }
            ...
            thisRef.open().then(
                    function (db) {
                        ...
                        transaction.onaboard = function (error) {
                            SWAC_debug.addErrorMessage('LocalFilesDB', 'Transaction for reading all files aboarded: ' + error);
                        };
                        let objectStore = transaction.objectStore("files");
                        ...
                        request.onerror = function (event) {
                            SWAC_debug.addErrorMessage('LocalFilesDB', 'Could not get datasets: ' + event);
                            reject();
                        };
