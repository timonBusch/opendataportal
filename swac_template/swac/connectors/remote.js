...
remoteHandler.apiCheckupURL = "zsystemservice/apiCheckup";
...
// Clear syncstates
    if (window.location.search.indexOf('clearsyncstates=true') > 0) {
        ...
        SWAC_debug.addDebugMessage('remote.js', 'Cleared syncstate chache.');
    }
});
...
remoteHandler.apiCheckup = function () {
    return new Promise((resolve, reject) => {
        // Disable api checkup
        if (SWAC_config.apicheckup === false) {
            resolve();
            return;
        }
        ...
        remoteHandler.fetchGet(remoteHandler.apiCheckupURL, querydata, true).then(function (json) {
            if (SWAC_config.debugmode) {
                SWAC_debug.addDebugMessage('remote', "apiCheckup succsessfull. API is reachable and answered with the following data:");
                console.log(json);
            }
                ...
                function (response) {
                    ...
                    if (!response.ok) {
                        if (!supressErrorMessage) {
                            ...
                            SWAC_debug.addDebugMessage('remote',
    ...
    if (!resource.startsWith('http')
        ...
        for (let datasource of SWAC_config.datasources) {
            ...
            if (typeof syncstates[url] === 'undefined') {
                ...
                SWAC_debug.addDebugMessage('remote.js', 'URL >' + url
