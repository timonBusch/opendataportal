...
SWAC.loadGlobalComponents = function () {
                    ...
                    .then(function (registration) {
                        ...
                        if (serviceWorker) {
                            ...
                            serviceWorker.addEventListener('statechange', function (e) {
                                console.log('ServiceWorker changed state:');
    ...
    SWAC.loadDependenciesStack(dependencyStack, corecomp).then(
            function () {
                // Check if debugmode should be activated
                let debugmode = SWAC.getParameterFromURL('debugmode', window.location);
                if (debugmode === 'true') {
                ...
                SWAC_language.init().then(function () {
                    ...
                    remoteHandler.apiCheckup().then(function (checkupresult) {
                        ...
                        SWAC_debug.addErrorMessage("swac", "apiCheckup failed with status: " + error);
                    });
                });

                if (SWAC_config.debugmode) {
                    // Insert debugging elements to page
                    SWAC_debug.addDebuggingElements();
                    // Show SWAC Version information
                    SWAC_debug.addDebugMessage('swac', 'Running on SWAC ' + SWAC.version);
                }
            }
    ).catch(function (error) {
        SWAC_debug.addErrorMessage('swac', 'Could not load a required file: ' + error);
...
SWAC.detectRequestors = function () {
    ...
    for (let requestor of requestors) {
        // load component
        componentHandler.loadComponent(requestor);
    }
};
...
SWAC.loadDependencies = function (component) {
    return new Promise((resolve, reject) => {
        if (component.requestor) {
            SWAC_debug.addDebugMessage('swac', 'Load dependencies for >'
                    + component.componentName + '< from requestor >'
                    + component.requestor.id + '<');
        } else {
            SWAC_debug.addDebugMessage('swac', 'Load dependencies for >'
                    + component.componentName + '< without requestor >');
        }
        ...
        SWAC.loadDependenciesStack(dependencyStack, component).then(function () {
            if (SWAC.checkDependenciesLoaded(component)) {
                ...
                SWAC_debug.addErrorMessage('swac',
                        'Could not load all dependencies from >'
                        + component.componentName + '<', component.requestor);
                reject();
            }
        });
    });
        ...
        if (typeof dependency !== 'undefined') {
            ...
            if (!skipImport) {
                ...
                if (SWAC.loadingdependencies.includes(depid)) {
                    SWAC_debug.addDebugMessage('SWAC', 'Waiting for loading dependency >' + depid + '<');
                ...
                if (dependency.component) {
                    ...
                    SWAC_debug.addErrorMessage('swac.js', 'Dependency >' + dependency.name + '< for >' + component.name + '< has unkown type and cannot be included.');
...
SWAC.onDependencyLoaded = function (loadid, dependency, dependenciesStack, addPromResolve) {
    ...
    SWAC_debug.addDebugMessage('swac.js', 'Loaded dependency >' + depName + '<');
...
SWAC.addDatasource = function (datasource_url) {
    if (SWAC.isValidURL(datasource_url)) {
        ...
        SWAC_debug.addErrorMessage('swac.js', datasource_url + " is not a valid url");
    }
};
...
SWAC.addAlgosource = function (algosource_url) {
    if (SWAC.isValidURL(algosource_url)) {
        ...
        SWAC_debug.addErrorMessage('swac.js', algosource_url + " is not a valid url");
    }
};
    ...
    performReactions() {
        ...
        for (let i in this.reactions) {
            ...
            for (let requiredRequestor of curReaction.requiredRequestors) {
                ...
                if (requestor === null) {
                    SWAC_debug.addErrorMessage('swac.js', 'The requestor >' + requiredRequestor
