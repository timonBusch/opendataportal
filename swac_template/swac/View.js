class View {
    ...
    loadTemplate() {
        ...
        let basePath = SWAC_config.swac_root + '/swac/components/' + this.component.componentName + "/";
        ...
        return new Promise((resolve, reject) => {
            ...
            if (template && template.style) {
                SWAC_debug.addDebugMessage('view', 'Load CSS >' + template.style + '< for >' + this.component.componentName + '<');
            ...
            if (this.requestor.childNodes.length > 1) {
                SWAC_debug.addDebugMessage('View', 'Useing onpage template for >' + this.requestor.id + '<', this.requestor);
                ...
                fetch(basePath + template.name + ".html").then(function (response) {
                    ...
                    SWAC_debug.addErrorMessage('view', 'Could not load HTML fragment >'
                            + template.name + '< for component >' + thisRef.component.componentName + "< : " + error, thisRef.requestor);
                    reject();
                });
            } else {
                SWAC_debug.addDebugMessage('view', 'There is no template for component >' + this.component.componentName + "<", this.requestor);
                resolve();
            }
        });
    }
     ...
     */
    prepareTemplate() {
        // Return object with bindpoints, repeatableForSets and repeatableForValues
        // only direct accessable ones
        let result = {};
    ...
    translateTemplate() {
        ...
        if (langmarks === null) {
            SWAC_debug.addDebugMessage('view', 'Found no language marks in template >'
                    + this.component.componentName + '/' + this.requestor.templateName + '.html<');
            return this.templatecode;
        }
        SWAC_debug.addDebugMessage('view', 'Found ' + langmarks.length
        ...
        for (let i in langmarks) {
            ...
            for (let j in curLangmarkPath) {
                ...
                if (langPath[curLangPathAdd]) {
                    ...
                    SWAC_debug.addDebugMessage('view', 'Language entry >' + curLangPathAdd + '< not found.');
                    break;
                }
            ...
            if (typeof repeatableElem !== 'function') {
                ...
                if (nodeName === 'TR') {
                    ...
                    if (repeatableElem.parentNode !== null) {
                        ...
                        SWAC_debug.addDebugMessage('view', 'The given repeatableElem has no parent.');
                    }
                }
            }
    }
    ...
    findChildsRepeatableForSet(element) {
        ...
        if (typeof element.nodeType === 'undefined') {
            SWAC_debug.addErrorMessage('view', 'The given object for findRepeatableForSet() is not an element');
    ...
    createRepeatedForSet(set, template) {
        if (!template || !template.classList) {
            SWAC_debug.addErrorMessage('view', 'No template given.');
            return;
        }
        if (!template.classList.contains('swac_repeatForSet')) {
            SWAC_debug.addErrorMessage('view', 'Given element is no template');
        }
    ...
    findChildsRepeatableForValue(element, setNo) {
        ...
        if (typeof element.nodeType === 'undefined') {
            SWAC_debug.addErrorMessage('view', 'The given object for findChildsRepeatableForValue() is not an element');
    ...
    findRepeatableForValue(element) {
        if (typeof element.nodeType === 'undefined') {
            SWAC_debug.addErrorMessage('view', 'The given object for findRepeatableForValue() is not an element');
    ...
    createRepeatedForValue(attr, set, template) {
        SWAC_debug.addDebugMessage('View', 'Create repeatForValue >' + attr + '< of set.id = >' + set.id + '<', this.requestor);
        let clone = template.cloneNode(true);
        ...
        if (clone.swac_bindPoints.hasOwnProperty('*') && attr.startsWith('swac_')) {
            SWAC_debug.addDebugMessage('View', 'Dont create askerik repeatForValue for metadata >' + attr + '<');
            return;
        }
    ...
    findBindPoints(requestorElem, stopatelementclass) {
        ...
        if (requestorElem.nodeType === 1 && requestorElem.getAttribute('swac_bp')) {
            ...
            let bindPoint = new BindPoint(markname);
    ...
    orderChildElements() {
        ...
        for (let child of allchilds) {
            ...
            if (!parentRef.startsWith("{")) {
                ...
                let parentid = SWAC_model.getIdFromReference(parentRef);
                ...
                if (parentRefWithoutId !== curRefLink) {
                    ...
                    SWAC_debug.addDebugMessage('view', msg, child);
                    continue;
                }
                ...
                if (parentElem === null) {
                    SWAC_debug.addDebugMessage('view', 'No parentElem for parent >' + parentid + '< found.');
                    continue;
                }
                // Get area where to place childs
                let parentsChildArea = this.findForChildsArea(parentElem);
                if (parentsChildArea === null) {
                    SWAC_debug.addDebugMessage('view', 'Could not find swac_forChilds element in parent >' + parentid + '<');
                    continue;
                }
                ...
                if (childsAsChildElem === null) {
                    SWAC_debug.addDebugMessage('view', 'Could not find swac_child for display content as child for >' + childid + '<');
                    continue;
                }
    ...
    findForChildsArea(element) {
        if (element === null) {
            SWAC_debug.addErrorMessage('view', 'Provided parameter >element< is null');
            return;
        }
