...
class OnlineReactions {
    constructor(config) {
        ...
        Promise.all(loadPromises).then(
                function () {
                    ...
                    SWAC_debug.addDebugMessage('OnlineReactions', 'SWAC OnlineReactions is ready with ' + loadPromises.length + ' reactions.');
                }
        ).catch(
                function (error) {
                    SWAC_debug.addErrorMessage('OnlineReactions', 'Could not load all OnlineReactions: ' + error);
                }
        );
    }
    ...
    reactOnCurrentState(evt) {
        ...
        switch (state) {
            ...
            case 'online':
                ...
                break;
            default:
                SWAC_debug.addErrorMessage('OnlineReactions', 'State >' + state + '< is unkown.');
        }
    }
     ...
     */
    onOnline(evt, proven) {
        SWAC_debug.addDebugMessage('OnlineReactions', 'Application is now online');
        this.hideOfflineMessage();
        for (let curReaction of this.reactions) {
            if (typeof curReaction.onOnline === 'function') {
                curReaction.onOnline(evt);
            } else {
                SWAC_debug.addErrorMessage('OnlineReactions', 'Reaction >' + curReaction.path + '< has no onOnline reaction.');
            }
        }
    }
     ...
     */
    onOffline(evt) {
        SWAC_debug.addDebugMessage('OnlineReactions', 'Application is now offline');
        ...
        for (let curReaction of this.reactions) {
            if (typeof curReaction.onOffline === 'function') {
                ...
                SWAC_debug.addErrorMessage('OnlineReactions', 'Reaction >' + curReaction.path + '< has no onOffline reaction.');
            }
        }
    }
    ...
    onUnreachable(evt) {
        ...
        for (let curReaction of this.reactions) {
            if (typeof curReaction.onUnreachable === 'function') {
                ...
                SWAC_debug.addErrorMessage('OnlineReactions', 'Reaction >' + curReaction.path + '< has no onUnreachable reaction.');
            }
        }
    }
    ...
    isReachable(url) {
        ...
        return fetch(url, {method: 'HEAD', mode: 'no-cors'})
                ...
                .catch(function (err) {
                    SWAC_debug.addDebugMessage('OnlineReactions', 'URL >' + url + 'is not reachable: ' + err);
                });
    }
            ...
            SWAC.loadDependenciesStack(dependencyStack, component).then(
                    function () {
                        ...
                        if (window[className + 'Factory']) {
                            ...
                            SWAC_debug.addDebugMessage('OnlineReactions', 'Reaction >' + className + '< created.');
                            resolve();
                        } else {
                            let msg = 'Script for onlinereaction >' + className + '< does not contain class.';
                            SWAC_debug.addErrorMessage('OnlineReactions', msg);
                            reject(msg);
                        }
