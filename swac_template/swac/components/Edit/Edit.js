...
The function gets the droped dataset and the dropzone element.'
        ...
        this.desc.opts[9] = {
            ...
            desc: 'If ture the definition interface /source/definition will be used to fetch definitions for building forms.'
        };
    ...
    init() {
        return new Promise((resolve, reject) => {
            ...
            if (addElem !== null && this.options.allowedToAddNew) {
                ...
                SWAC_debug.addDebugMessage('Edit', 'Adding new objects is not permitted for >'
                        + this.requestor.id + '<. Deactivateing add button. You can activate '
                        + 'the button by adding allowedToAddNew=true to the options.',
                        this.requestor);
            } else {
                SWAC_debug.addDebugMessage('Edit', 'There is no add button >.swac_editAddSetButton< in the template.', this.requestor);
            }
    ...
    afterAddSet(fromName, set) {
        ...
        if (existingSet !== null) {
            SWAC_debug.addDebugMessage('edit', "Set with unique attribute >" + set[this.options.uniqueSetAttribute] + '< already exists.');
            return;
        }
    ...
    transformRepeated(inputAreaElem) {
        ...
        if (this.options.definitions.size > 0) {
            ...
            SWAC_debug.addDebugMessage('Edit', 'Useing plain text inputs, because there are no definitions.', this.requestor);
        }
    }
    ...
    createMatchingInputElement(attrDef, set) {
        ...
        SWAC_debug.addDebugMessage('Edit', msg, this.requestor);
    ...
    createReferenceDisplay(ref, reftableElem)
    {
        ...
        SWAC_model.getFromReference(ref).then(function (dataCapsle) {
            ...
            SWAC_debug.addErrorMessage('edit', 'Could not get data for ref >' + ref + '<: ' + error);
            console.log(error);
        });
    }
    ...
    onClickAddButton(evt) {
        ...
        if (!inputAreaElemTpl) {
            SWAC_debug.addErrorMessage('Edit', 'There is no input area available (this is a .swac_repeatForSet).', this.requestor);
            return;
        }
        let inputAreaElem;
    ...
    showInputArea(inputAreaElem, appendToElem) {
        ...
        if (editorTarget !== null) {
            this.hideInputArea();
            SWAC_debug.addDebugMessage('Edit', 'Insert edit inputs into target >' + this.options.editorTargetElement + '<');
            // Note displaystate
            ...
            SWAC_debug.addDebugMessage('Edit', 'Append edit input to given element because \n\
                there is no options.editorTargetElement specified.');
            inputAreaElem.classList.remove('swac_dontdisplay');
            appendToElem.appendChild(inputAreaElem);
        } else if (!inputAreaElem.parentElement && appendToElem) {
            SWAC_debug.addDebugMessage('Edit', 'Append edit input to requestor because \n\
                there is no options.editorTargetElement specified.');
            this.requestor.appendChild(inputAreaElem);
        } else if (inputAreaElem.classList.contains('swac_dontdisplay')) {
            SWAC_debug.addDebugMessage('Edit', 'Makeing edit input visible at the place its defined in template.');
            inputAreaElem.classList.remove('swac_dontdisplay');
        } else {
            SWAC_debug.addDebugMessage('Edit', 'Edit input is allready visible.');
        }
    }
    ...
    hideInputArea() {
        ...
        // Reset and insert editor form
        if (editorTarget !== null) {
            SWAC_debug.addDebugMessage('Edit', 'Remove inputs from target >' + this.options.editorTargetElement + '<');
    ...
    onClickSave(evt) {
        ...
        if (formElem.reportValidity()) {
            SWAC_debug.addDebugMessage('edit', 'Input in form >' + formElem.id + '< was validated succsessfully.');
    ...
    saveObject(formElem, supressMessages) {
        SWAC_debug.addDebugMessage('edit', 'Save information from form >' + formElem.id + '<');
        ...
        for (let i in formElem.elements) {
            ...
            if (typeof formElem.elements[i] !== 'function'
                ...
                if (formElem.elements[i].nodeName === 'INPUT') {
                    ...
                    SWAC_debug.addErrorMessage('Edit', 'Support for ' + formElem.elements[i].nodeName + " not implemented yet.");
                }
                if (value !== null) {
                ...
                if (nodeName !== 'BUTTON'
                        && nodeName !== 'FIELDSET'
                        && i !== 'length') {
                    SWAC_debug.addDebugMessage('Edit',
    ...
    onClickSaveAll(evt) {
        ...
        if (onefailed) {
            ...
            SWAC_debug.addDebugMessage('edit', 'Input in all forms was validated succsessfully.');
        }
    ...
    onDrop(evt) {
        ...
        // validate form
        if (formElem.reportValidity()) {
            SWAC_debug.addDebugMessage('edit', 'Input in form >' + formElem.getAttribute('id') + '< was validated succsessfully.');
        ...
        SWAC_model.getFromReference(ref).then(function (set) {
            ...
            if (dropfunc) {
                ...
                try {
                    ...
                    SWAC_debug.addErrorMessage('edit', 'Could not execute external drop-function >' + dropfunc.name + '< for ref >' + ref + '<: ' + error);
                }
            } else if (joinerTarget) {
                thisRef.createJoiner(set, joinerTarget, evt.target, formElem);
            } else {
                // Create list entry
                SWAC_debug.addErrorMessage('edit', 'Default drop generation is not implemented yet.');
            }
        }).catch(function (error) {
            SWAC_debug.addErrorMessage('edit', 'Could not fetch data for reference >' + ref + '<: ' + error);
    ...
    createJoiner(set, joinerDef, dropzoneElem, formElem) {
        ...
        SWAC_debug.addDebugMessage('Edit', 'Send data to source >' + joinerDef.targetSetName + '<', this.requestor);
        // Save joiner
        let thisRef = this;
