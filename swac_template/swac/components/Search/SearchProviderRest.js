...
class SearchProviderRest extends SearchProvider {
    ...
    search(searchExpr, searchurl, searchcomp) {
        SWAC_debug.addDebugMessage('SearchProviderRest', 'Now searching on >' + searchurl + '<', searchcomp.requestor);
        ...
        return new Promise((resolve, reject) => {
            if (searchExpr.length < minchars) {
                SWAC_debug.addDebugMessage('SearchProviderRest', 'Do not perform search, because there are less than ' + minchars + ' chars', searchcomp.requestor);
            ...
            if (searchcomp.searchFor !== searchExpr) {
                SWAC_debug.addDebugMessage('SearchProviderRest', '>' + searchExpr + '< is no longer searched.');
                resolve();
                return;
            }
            ...
            fetch(searchpath).then(function (response) {
                if (response.ok) {
                    ...
                    SWAC_debug.addDebugMessage('SearchProviderRest', 'Nothing found on ' + searchpath);
                    resolve([]);
                } else {
