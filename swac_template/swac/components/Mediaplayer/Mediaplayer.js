...
class Mediaplayer extends Component {
    ...
    constructor(options = {}) {
        super(options);
        this.componentName = 'Mediaplayer';
    ...
    afterAddSet(fromName, set) {
        ...
        if (set.alternative) {
            ...
            if (set.id === set.alternative) {
                SWAC_debug.addErrorMessage('Mediaplayer', 'Media >' + set.id + '< references himself as alternatie.', this.requestor);
                return;
            }
        }
                ...
                if (!set.alternative) {
                    ...
                    if (mediaElem) {
                        ...
                        mediaElem.addEventListener('loadedmetadata', function () {
                            ...
                            SWAC_debug.addDebugMessage('Mediaplayer', 'Added media >'
        ...
        SWAC_model.load(this.options.commentRequestor).then(
                function (result) {
                    for (let curComment of result.data) {
                        ...
                        if (!media) {
                            SWAC_debug.addDebugMessage('Mediaplayer',
                ...
                function (error) {
                    SWAC_debug.addErrorMessage('Mediaplayer', 'Could not load comments >' + error + '<');
                }
        );
    }
    ...
    openOverlay(overlaydata, overlayset) {
        ...
        // Generate media
        let mediaCode = new Mediacode(overlaydata, overlayset, this.options.pathattr, 'mimetype');
