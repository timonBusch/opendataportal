...
class VisucreatorSPL extends ComponentPlugin {
    ...
    constructor(pluginconf) {
        super(pluginconf);
        this.componentName = 'Visualmodel/plugins/visucreator';
    ...
    init() {
        ...
        this.stage.on("click", this.focusVisuelement.bind(this));
        // .. for buttons
        let clearBtn = this.requestor.querySelector('.swac_visualmodel_clear');
        clearBtn.addEventListener('click', this.onClear.bind(this));

        let removeBtn = this.requestor.querySelector('.swac_visualmodel_remove');
        removeBtn.addEventListener('click', this.onRemove.bind(this));

        let addElemBtn = this.requestor.querySelector('.swac_visualmodel_addelement');
        addElemBtn.addEventListener('click', this.onAddElem.bind(this));

        let copyElemBtn = this.requestor.querySelector('.swac_visualmodel_copyelement');
        copyElemBtn.addEventListener('click', this.onCopyElem.bind(this));

        let addConnectorBtn = this.requestor.querySelector('.swac_visualmodel_addconnector');
        addConnectorBtn.addEventListener('click', this.onAddConnector.bind(this));
...
//        });
    ...
    onRemove() {
        ...
        if (!this.visuelemFocus) {
            UIkit.modal.alert(SWAC_language.Visualmodel.visucreator.remove_nonsel);
            return;
        }
    ...
    onCopyElem() {
        // Check if an element is selected
        if (!this.visuelemFocus) {
            UIkit.modal.alert(SWAC_language.Visualmodel.visucreator.copy_nonsel);
            return;
        }
    ...
    onAddConnector() {
        // Check if an element is selected
        if (!this.visuelemFocus) {
            UIkit.modal.alert(SWAC_language.Visualmodel.visucreator.con_nonsel);
            return;
        }
        ...
        // Inform user
        UIkit.modal.alert(SWAC_language.Visualmodel.visucreator.con_selpart2);
    }
