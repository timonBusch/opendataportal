 ...
 */
class Thermometer extends Diagram {
    constructor(unit, name, width, height, datadescription, diagramDef) {
        super(unit, name, width, height, datadescription, diagramDef);
    ...
    drawValueDiagram(value) {
        if (!this.datadescription) {
            SWAC_debug.addErrorMessage('Thermometer', 'There is no datadescription defined.');
            return;
        }
        ...
        for (let curDef of definitions) {
            ...
            if (curDef.maxValue > maxValue) {
                SWAC_debug.addDebugMessage('Thermometer', 'Definitions maxValue >'
            ...
            if (curDef.minValue < minValue) {
                SWAC_debug.addDebugMessage('Thermometer', 'Definitions minValue >'
...
//            let barpartLowPos = lowPosOnBar + this.barTopPos;
        ...
        if (value > maxValue) {
            SWAC_debug.addDebugMessage('Thermometer', 'Value >'
...
/**
     ...
     */
    drawSetDiagram(set) {
        SWAC_debug.addErrorMessage('Diagram', 'Thermometer does not support visualising of datasets.');
    }
}

diagram_types['Thermometer'] = Thermometer;