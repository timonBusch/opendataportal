...
CommunicatorFactory.create = function (config) {
...
class Communicator extends Component {
    ...
    constructor(options = {}) {
        super(options);
        this.componentName = 'Communicator';

        this.desc.text = 'Component allowing per to per communication.';
        this.desc.depends[0] = {
            name: 'adapter.js',
            path: SWAC_config.swac_root + '/swac/components/Communicator/libs/adapter.js',
            desc: 'Adapter for WebRTC.'
        };
        this.desc.depends[1] = {
            name: 'peerjs.js',
            path: SWAC_config.swac_root + '/swac/components/Communicator/libs/peerjs.min.js',
...
//        videoelem.srcObject = mediaStream;
    ...
    onChangeScreencapState(evt) {
        ...
        navigator.mediaDevices.getDisplayMedia(displayMediaOptions).then(
                ...
                function (error) {
                    SWAC_debug.addErrorMessage('Communicator', 'Could not start screencapture: ' + error, thisRef.requestor);
                });
    }
    ...
    onGoOnline(evt) {
        evt.preventDefault();
        this.requestor.swac_view.insertLoadingElem(SWAC_language.Communicator.goingonline);
        let thisRef = this;
        // Get username
        ...
        if (username === '') {
            UIkit.modal.alert(SWAC_language.Communicator.missingusername);
            return;
        }
        this.peer = new Peer(
                username,
...
//                            }
        ...
        this.peer.on('open', function (id) {
            thisRef.requestor.swac_view.removeLoadingElem();
            // Show message
            let msg = SWAC_language.replacePlaceholders(SWAC_language.Communicator.youreonlinenow, 'id', id);
        ...
        this.peer.on('error', function (error) {
            ...
            if (error.type === 'unavailable-id') {
                let msg = SWAC_language.replacePlaceholders(SWAC_language.Communicator.usernametaken, 'username', username);
                UIkit.modal.alert(msg);
            } else {
                UIkit.modal.alert(SWAC_language.Communicator.unkownerror);
                SWAC_debug.addErrorMessage('Communicator', 'An >' + error.type + '< error occured.', thisRef.requestor);
            }
        });
    }
    ...
    onConnectToUser(evt) {
        ...
        if (!this.peer) {
            UIkit.modal.alert(SWAC_language.Communicator.notonline);
            return;
        }

        let comRemoteuserElem = this.requestor.querySelector('[name="com_remoteuser"]');
        if (comRemoteuserElem.value === '') {
            UIkit.modal.alert(SWAC_language.Communicator.noremoteuser);
            return;
        }
        ...
        chatconnection.on('error', function (error) {
            SWAC_debug.addErrorMessage('Communicator', 'Error in connection to ' + uname + ': ' + error);
        });
...
//        });
    ...
    onSendChat(evt) {
        ...
        if (this.connections.size === 0) {
            UIkit.modal.alert(SWAC_language.Communicator.noconnection);
            return;
        }
        // Get message
...
//        this.connection.onsignalingstatechange = handleSignalingStateChangeEvent;
            ...
            SWAC_debug.addErrorMessage('Communicator', "Can't establish connection: " + error, this.requestor);
        }
    }
