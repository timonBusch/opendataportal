var MapselectFactory = {};
MapselectFactory.create = function (config) {
    return new Mapselect(config);
};
...
class Mapselect extends Component {
    ...
    constructor(options = {}) {
        ...
        this.desc.depends[0] = {
            name: 'leaflet.js',
            path: SWAC_config.swac_root + '/swac/components/Mapselect/libs/leaflet.js',
            desc: 'Leaflet library for viewing maps.'
        };
        this.desc.depends[1] = {
            name: 'leaflet.css',
            path: SWAC_config.swac_root + '/swac/components/Mapselect/libs/leaflet.css',
            desc: 'Style for leafelet maps.'
        };
        this.desc.depends[2] = {
            name: 'leaflet.draw.js',
            path: SWAC_config.swac_root + '/swac/components/Mapselect/libs/leaflet.draw.js',
            desc: 'Extension for leaflet that allows drawing on the map.'
        };
        this.desc.depends[3] = {
            name: 'leaflet.draw.css',
            path: SWAC_config.swac_root + '/swac/components/Mapselect/libs/leaflet.draw.css/leaflet.draw.css',
        ...
        if (placeholders) {
            for (const match of placeholders) {
                if (replaces.has(match)) {
                    ...
                    SWAC_debug.addErrorMessage('Mapselect', 'Unkown placeholder >'
