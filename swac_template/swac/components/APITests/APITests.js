var APITestsFactory = {};
APITestsFactory.create = function (config) {
    return new APITests(config);
};
...
class APITests extends Component {
    ...
    constructor(options = {}) {
        super(options);
        this.componentName = 'APITests';

        this.desc.text = 'Component for testing data APIs';
        this.desc.depends[0] = {
            name: 'TreeQLTests.js',
            path: SWAC_config.swac_root + '/swac/components/APITests/TreeQLTests.js',
...
//        }
    ...
    testAll(url) {
        ...
        let setElem = this.requestor.swac_view.findReapeatedForSet(urlinputElem);
    ...
    showTestresult(url, testname, state, result) {
        ...
        if (typeof result.json === 'function') {
            result.json().then(function (json) {
                let rtext = SWAC_language.APITests.restapierr;
                if (json.errors) {
                ...
                testresultElem.innerHTML = rtext + '<br>' + ' <a href="'
                        + url + '" target="_blank">' +
                        SWAC_language.APITests.showinbrowser
                        + '</a>';
            }).catch(function () {
