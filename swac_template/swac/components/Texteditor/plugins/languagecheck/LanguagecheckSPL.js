var languagecheckFactory = {};
languagecheckFactory.create = function (pluginconfig) {
    return new LanguagecheckSPL(pluginconfig);
};

/**
 * Plugin for connecting informations from various datasources with text
 */
class LanguagecheckSPL extends ComponentPlugin {

    constructor(pluginconf) {
        super(pluginconf);
        this.componentName = 'texteditor/plugins/languagecheck';
        this.desc.depends[0] = {
...
//        this.options.showdatalabels = false;
    ...
    init() {
        ...
        if (!this.contElements || this.contElements.length === 0) {
            SWAC_debug.addErrorMessage('LanguagecheckSPL', 'This plugin needs a contElement to insert the chart.', this.requestor);
        }

        // Register event handlers for updateing displys when changeing tab
        let docSwitcher = this.requestor.querySelector('.swac_texteditor_docswitch');
        if (docSwitcher) {
        ...
        let calcFREBtn = this.requestor.querySelector('.swac_texteditor_quality_fre_recalc');
        if (calcFREBtn) {
            calcFREBtn.addEventListener('click', this.onRecalcFRE.bind(this));
        }
        let calcWSFBtn = this.requestor.querySelector('.swac_texteditor_quality_wsf_recalc');
        if (calcWSFBtn) {
            calcWSFBtn.addEventListener('click', this.onRecalcWSF.bind(this));
        }
        let calcINTBtn = this.requestor.querySelector('.swac_texteditor_quality_int_recalc');
        if (calcINTBtn) {
    ...
    showStatistic(text, lang) {
        ...
        let langElem = this.requestor.querySelector('.swac_texteditor_lang');
        if (langElem)
            langElem.innerHTML = SWAC_language.Texteditor.languagecheck['lang_' + this.ta.getLang()];

        let stat_charsElem = this.requestor.querySelector('.swac_texteditor_stat_chars');
        if (stat_charsElem)
            stat_charsElem.innerHTML = this.ta.getCharsLength();

        let stat_sentencesElem = this.requestor.querySelector('.swac_texteditor_stat_sentences');
        if (stat_sentencesElem)
            stat_sentencesElem.innerHTML = this.ta.getSentencesLength();

        let words = this.ta.getWordsLength();
        let stat_wordsElem = this.requestor.querySelector('.swac_texteditor_stat_words');
        if (stat_wordsElem)
            stat_wordsElem.innerHTML = words;

        let means = this.ta.getSentencesStatsMeans();

        let stat_wordsPerSentenceElem = this.requestor.querySelector('.swac_texteditor_stat_wordspersentence');
        if (stat_wordsPerSentenceElem)
            stat_wordsPerSentenceElem.innerHTML = means.aMeanWordcount.toFixed(2);

        let stat_partsPerSentenceElem = this.requestor.querySelector('.swac_texteditor_stat_partspersentence');
        ...
        let repeatedForWordsElems = this.requestor.querySelectorAll('.swac_texteditor_repeatedForWord');
        for (let curRepForWordElem of repeatedForWordsElems) {
            curRepForWordElem.parentNode.removeChild(curRepForWordElem);
        }

        let repeatForWordElem = this.requestor.querySelector('.swac_texteditor_repeatForWord');
        ...
        let freElem = this.requestor.querySelector('.swac_texteditor_quality_readability_fre');
        if (freElem)
            freElem.innerHTML = SWAC_language.Texteditor.languagecheck['fre_level_' + this.ta.calcFRELevel(this.options.fre_factors)];
        let fredetElem = this.requestor.querySelector('.swac_texteditor_quality_fre_details');
        ...
        let freoptElem = this.requestor.querySelector('.swac_texteditor_quality_fre_options');
        ...
        let wsfElem = this.requestor.querySelector('.swac_texteditor_quality_readability_wsf');
        if (wsfElem)
            wsfElem.innerHTML = SWAC_language.Texteditor.languagecheck['wsf_level_' + this.ta.calcWSFLevel()];
        let wsfdetElem = this.requestor.querySelector('.swac_texteditor_quality_wsf_details');
        ...
        let wsfoptElem = this.requestor.querySelector('.swac_texteditor_quality_wsf_options');
        ...
        let intliElem = this.requestor.querySelector('.swac_texteditor_quality_intelligibility');
        if (intliElem)
            intliElem.innerHTML = SWAC_language.Texteditor.languagecheck['wsf_level_' + this.ta.calcIntelligibilityLevel(this.options.int_factors)];
        let intdetElem = this.requestor.querySelector('.swac_texteditor_quality_int_details');
        ...
        let intoptElem = this.requestor.querySelector('.swac_texteditor_quality_int_options');
        ...
        let tqapElem = this.requestor.querySelector('.swac_texteditor_quality_tqap');
        if (tqapElem)
            tqapElem.innerHTML = SWAC_language.Texteditor.languagecheck['tqap_level_' + this.ta.calcTQAPLevel()];

        // Create list of keywords
        let repeatForKeywordsElem = this.requestor.querySelector('.swac_texteditor_repeatForKeyword');
        if (repeatForKeywordsElem) {
            // Remove keywords from old calculation
            let repeatedsForKeyElems = this.requestor.querySelectorAll('.swac_texteditor_repeatedForKeyword');
    ...
    resetStatistic() {
        let langElem = this.requestor.querySelector('.swac_texteditor_lang');
        if (langElem)
            langElem.innerHTML = SWAC_language.Texteditor.languagecheck.lang_none;
        ...
        let freElem = this.requestor.querySelector('.swac_texteditor_quality_readability_fre');
        if (freElem)
            freElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
        let fredetElem = this.requestor.querySelector('.swac_texteditor_quality_fre_details');
        if (fredetElem && this.options.show_details) {
            let aslElem = fredetElem.querySelector('.swac_texteditor_quality_fre_asl');
            aslElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
            let aswElem = fredetElem.querySelector('.swac_texteditor_quality_fre_asw');
            aswElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
        } else {
            fredetElem.parentNode.removeChild(fredetElem);
        }
        let freoptElem = this.requestor.querySelector('.swac_texteditor_quality_fre_options');
        if (freoptElem && this.options.show_options) {
            let baseElem = freoptElem.querySelector('#swac_texteditor_quality_fre_base');
            baseElem.value = SWAC_language.Texteditor.languagecheck.notcalculated;
            let aslElem = freoptElem.querySelector('#swac_texteditor_quality_fre_aslfactor');
            aslElem.value = SWAC_language.Texteditor.languagecheck.notcalculated;
            let aswElem = freoptElem.querySelector('#swac_texteditor_quality_fre_aswfactor');
            aswElem.value = SWAC_language.Texteditor.languagecheck.notcalculated;
        } else {
            freoptElem.parentNode.removeChild(freoptElem);
        }
        ...
        let wsfElem = this.requestor.querySelector('.swac_texteditor_quality_readability_wsf');
        if (wsfElem)
            wsfElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
        let wsfdetElem = this.requestor.querySelector('.swac_texteditor_quality_wsf_details');
        if (wsfdetElem && this.options.show_details) {
            let msElem = wsfdetElem.querySelector('.swac_texteditor_quality_wsf_ms');
            msElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
            let iwElem = wsfdetElem.querySelector('.swac_texteditor_quality_wsf_iw');
            iwElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
            let slElem = wsfdetElem.querySelector('.swac_texteditor_quality_wsf_sl');
            slElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
            let esElem = wsfdetElem.querySelector('.swac_texteditor_quality_wsf_es');
            esElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
        } else {
            wsfdetElem.parentNode.removeChild(wsfdetElem);
        }
        let wsfoptElem = this.requestor.querySelector('.swac_texteditor_quality_wsf_options');
        ...
        let intliElem = this.requestor.querySelector('.swac_texteditor_quality_intelligibility');
        if (intliElem)
            intliElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
        let intdetElem = this.requestor.querySelector('.swac_texteditor_quality_int_details');
        if (intdetElem && this.options.show_details) {
            let nwsElem = intdetElem.querySelector('.swac_texteditor_quality_int_nws');
            nwsElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
            let npsElem = intdetElem.querySelector('.swac_texteditor_quality_int_nps');
            npsElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
            let ncsElem = intdetElem.querySelector('.swac_texteditor_quality_int_ncs');
            ncsElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
            let arwElem = intdetElem.querySelector('.swac_texteditor_quality_int_arw');
            arwElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
            let afwElem = intdetElem.querySelector('.swac_texteditor_quality_int_afw');
            afwElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
            let nasElem = intdetElem.querySelector('.swac_texteditor_quality_int_nas');
            nasElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
        } else {
            intdetElem.parentNode.removeChild(intdetElem);
        }
        ...
        let tqapElem = this.requestor.querySelector('.swac_texteditor_quality_tqap');
        if (tqapElem)
            tqapElem.innerHTML = SWAC_language.Texteditor.languagecheck.notcalculated;
    ...
    onRecalcWSF(evt) {
        evt.preventDefault();
        let variantElem = this.requestor.querySelector('#swac_texteditor_quality_wsf_variant');
