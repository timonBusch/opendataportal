var QuestionFactory = {};
QuestionFactory.create = function (config) {
...
class Question extends Component {
    ...
    constructor(options = {}) {
        ...
        if (!options.afterSaveTxt)
            this.options.afterSaveTxt = SWAC_language.Question.aftersavetxt;
        this.desc.opts[4] = {
            name: "afterSaveLoc",
    ...
    init() {
        return new Promise((resolve, reject) => {
            ...
            for (let curSource in this.data) {
                let curSets = this.data[curSource];
    ...
    afterAddSet(fromName, set) {
        ...
        if (set.type !== 'group' && !set.attr) {
            SWAC_debug.addErrorMessage('Question', 'The needed attribute >attr< is missing at question >' + set.id + '<', this.requestor);
        }
        ...
        switch (set.type) {
            ...
            case 'datetime':
                ...
                break;
            default:
                SWAC_debug.addDebugMessage('Question', 'Questiontype >' + set.type + '< is not supported.');
        }
...
//        divElem.appendChild(document.createElement('br'));
    ...
    createInputForSelect(set) {
        ...
        if (!set.multiple) {
            ...
            notSelOptElem.innerHTML = SWAC_language.Question.pleaseselect;
    ...
    createInputForIcon(set) {
        ...
        for (let curOption of set.options) {
            ...
            if (curOption.value) {
                ...
                curOptionSrc = SWAC_config.swac_root + '/swac/components/Icon/imgs/' + curOption + '.svg';
            }
    ...
    onMadeInput(evt) {
        ...
        if (clickedElem.nodeName === 'IMG') {
            ...
            if (clickedElem.classList.contains('swac_question_icon')) {
                ...
                // Get setid
                let setname = setElem.getAttribute('swac_setname');
                let setno = setElem.getAttribute('swac_setno');
                let set = this.data[setname][setno];
...
//        this.nextQuestion();
    ...
    onSend(evt) {
        ...
        for (let curInputElem of formElem.elements) {
            ...
            // Get dataset
            let setname = setdiv.getAttribute('swac_setname');
            let setno = setdiv.getAttribute('swac_setno');
            let set = this.data[setname][setno];
            // Search noty element
            ...
            if (!curInputElem.checkValidity()) {
                ...
                if (curInputElem.validity.valueMissing) {
                    notyElem.innerHTML = SWAC_language.Question.valuemissing;
                } else if (curInputElem.validity.typeMismatch) {
                    notyElem.innerHTML = SWAC_language.Question.valuetypemismatch;
                } else if (curInputElem.validity.tooShort) {
                    let min = curInputElem.getAttribute('minlength');
                    let msg = SWAC_language.replacePlaceholders(SWAC_language.Question.valuetoshort, 'charsmin', min);
                    notyElem.innerHTML = msg;
                } else if (curInputElem.validity.tooLong) {
                    let max = curInputElem.getAttribute('maxlength');
                    notyElem.innerHTML = SWAC_language.Question.valuetolong;
                    let msg = SWAC_language.replacePlaceholders(SWAC_language.Question.valuetolong, 'charsmax', max);
                    notyElem.innerHTML = msg;
                } else if (curInputElem.validity.rangeOverflow) {
                    let max = curInputElem.getAttribute('max');
                    let msg = SWAC_language.replacePlaceholders(SWAC_language.Question.valuerangeoverflow, 'maxvalue', max);
                    notyElem.innerHTML = msg;
                } else if (curInputElem.validity.rangeUnderflow) {
                    let min = curInputElem.getAttribute('min');
                    let msg = SWAC_language.replacePlaceholders(SWAC_language.Question.valueramgeunderflow, 'minvalue', min);
                    notyElem.innerHTML = msg;
                } else if (curInputElem.validity.patternMismatch) {
                    notyElem.innerHTML = SWAC_language.Question.valuepatternmismatch;
                }
            ...
            if (!set.target) {
                ...
                let parentSetname = SWAC_model.getSetnameFromRefernece(set.parent);
                let parentId = SWAC_model.getIdFromReference(set.parent);
                // Search parent dataset
                let parentSet;
                for (let curSet of this.data[parentSetname]) {
            ...
            if (!target) {
                SWAC_debug.addErrorMessage('Question', 'There is no target defined for >' + set.id + '< data will not be saved.', this.requestor);
                continue;
            }
            ...
            if (!dataCapsles.has(target)) {
                ...
                dataCapsle.metadata.fromSource = SWAC_model.getSetnameFromRefernece(target);
        ...
        if (someInvalid) {
            UIkit.modal.alert(SWAC_language.Question.forgotteninput);
            return;
        }

        SWAC_debug.addDebugMessage('Question', 'Input in form was validated succsessfully.');
        ...
        for (let curName of dataCapsles.keys()) {
            ...
            let savePromise = SWAC_model.save(curDataCapsle);
        ...
        Promise.all(savePromises).then(
                ...
                function (error) {
                    SWAC_debug.addErrorMessage('Question', 'Some dataset could not be saved: ' + error);
                }
        );
    }
    ...
    afterSaveData(result) {
        ...
        // Show text
        let tnxTxt = SWAC_language.Question.aftersavetxt;
        ...
        if (this.options.afterSaveLoc) {
            let thisRef = this;
            msgElem.appendChild(document.createTextNode(' ' + SWAC_language.Question.redirection));
            setTimeout(function () {
        ...
        if (this.options.timeToReanswer !== null) {
            ...
            msgElem.appendChild(document.createTextNode(' ' + SWAC_language.Question.reanswerText));
