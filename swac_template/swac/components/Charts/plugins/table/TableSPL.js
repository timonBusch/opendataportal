...
class TableSPL extends ComponentPlugin {
    ...
    constructor(pluginconf) {
        super(pluginconf);
        this.componentName = 'chart/plugins/table';
    ...
    createTable(datasourceName) {
        SWAC_debug.addDebugMessage('TableSPL', 'Creating table for data from >' + datasourceName + '<', this.requestor);

        // Check if content area is available
        if (!this.contElements || this.contElements.length === 0) {
            SWAC_debug.addErrorMessage('TableSPL', 'This plugin needs a contElement to insert the chart.', this.requestor);
        }
