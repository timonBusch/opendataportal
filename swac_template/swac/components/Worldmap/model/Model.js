...
class Model {
        ...
        return new Promise((resolve, reject) => {
            ...
            fetch(this._filepath).then(
                    function (response) {
                        ...
                        if (typeof SWAC_storage.files[modelobj._filepath].metainfo === 'undefined') {
                            SWAC_storage.files[modelobj._filepath].metainfo = {};
                        }

                        SWAC_storage.files[modelobj._filepath].metainfo.filestatus = response.status;
                        if (response.status >= 400 && response.status < 600) {
                            reject("Model >" + modelobj._filepath + "< not accessable.");
                        } else {
                            response.json().then(function (data) {
                                SWAC_storage.files[modelobj._filepath].data = data;
                                SWAC_debug.addDebugMessage('Model', 'Model >'
    ...
    getFromStorage() {
        // Bind to storage
        if (typeof SWAC_storage.files[this._filepath] === 'undefined') {
            SWAC_storage.files[this._filepath] = {};
            SWAC_storage.files[this._filepath].url = this._filepath;
        }
        this.file = SWAC_storage.files[this._filepath];

        // Prevent model from loading more than once
        if (typeof SWAC_storage.files[this._filepath].data !== 'undefined') {
            return SWAC_storage.files[this._filepath].data;
        }
        return null;
    }
    ...
    getVisualValues(modeldata) {
        ...
        if (this.options.fillColorProperty
            ...
            SWAC_debug.addDebugMessage('Model', 'There is no description component bound and no visoptions set. Model will be colored with default values.');
        }
