 ...
 */
class GeneralDataJson extends Model {
    /**
    ...
    get locations() {
        ...
        if (this.options.latattr && this.options.lonattr) {
            ...
            SWAC_debug.addErrorMessage('GeneralDataJson', errmsg, this._requestor);
            throw(errmsg);
        }
    ...
    calculateLocationsFromAttributes() {
        ...
        if (missingLatLonSets.length > 0) {
            ...
            SWAC_debug.addErrorMessage('GeneralDataJson', 'Datasets ' + setNoStr
        ...
        return new Promise((resolve, reject) => {
            super.loadJson(filepath).then(function () {
                ...
                if (thisRef.file.data.list) {
                    SWAC_debug.addDebugMessage('GeneralJsonData', 'Detected list form data.', thisRef._requestor);
                ...
                SWAC_debug.addErrorMessage('GeneralDataJson', 'Error occured while loading >'
                        + filepath + '< ' + error);
                reject();
            });
        });
    ...
    draw(viewer) {
        ...
        return new Promise((resolve, reject) => {
            if (thisRef._drawnref.subs.lenght > 0) {
                SWAC_debug.addDebugMessage('GeneralDataJson', 'The model >' + thisRef + '< is allready drawn.');
            ...
            if (!thisRef.options.datadescription) {
                SWAC_debug.addDebugMessage('GeneralDataJson', 'There is no datadescription bound to this data. Please take into account, that you can only bound the datadescription to the datasource, after dom was loaded completely.');
            }
            ...
            if (thisRef._timeddata.size === 0 &&
                ...
                SWAC_debug.addDebugMessage('GeneralDataJson', 'Earasing previous drawn data.');
            }
            ...
            if (this._excludedAttrs.keys().length > 0) {
                ...
                SWAC_debug.addDebugMessage('GeneralDataJson', 'The attributes >'
    ...
    drawDataset(viewer, dataset, setno = 0) {
        SWAC_debug.addDebugMessage('GeneralDataJson', 'Drawing dataset no >'
    ...
    drawAttribute(viewer, attr, dataset, lat, lon, height) {
        ...
        if (this.options.displayKind === 'cylinder') {
            ...
            SWAC_debug.addErrorMessage('GeneralDataJson',
    ...
    drawDatalistTimed(viewer) {
        ...
        if (dataInterval) {
            ...
            if (this._timeddrawn !== dataTime) {
                ...
                SWAC_debug.addDebugMessage('GeneralDataJson', 'Dataset for >' + dataIsoTime + '< is allready drawn. Skipping draw.');
            }
        } else {
            SWAC_debug.addDebugMessage('GeneralDataJson', 'Did not found data for >' + curTime + '<', this._requestor);
        }
    }
    ...
    drawTooltip(movement) {
        ...
        if (typeof valueIdentifier[2] !== 'undefined') {
            ...
            if (this.options.tsattr && dataset[this.options.tsattr]) {
                ...
                tsElem.innerHTML = SWAC_language.Worldmap.datafrom + ' '
