...
class SearchEntryMakerHid extends SearchEntryMaker {
    ...
    isApplicable(searchResult) {
        ...
        SWAC_debug.addDebugMessage('SearchEntryMakerHid', 'Result with hid >' + searchResult.result + '< is not renderable with this maker.');
        return false;
    }
    ...
    make(searchResults, resultNo, placeElement) {
        ...
        fetch(this.groundpath).then(
            ...
            SWAC_debug.addErrorMessage('SearchEntryMakerHid', 'A error occured searching: ' + error);
            refCaller.searchModel(hid);
        });
    }
        ...
        fetch(modelpath, {method: 'HEAD'}).then(
                function (response) {
                    if (response.status < 400 || response.status === 405) {
                        ...
                        SWAC_debug.addDebugMessage('SearchEntryMakerHid', 
                        'There is no building model available for hid >' + hid + '<');
                    }
                    refCaller.createEntry(hid);
                }).catch(function (error) {
            SWAC_debug.addErrorMessage('SearchProviderFile', 'A error occured searching: ' + error);
        });
    }
 ...
 */
    createEntry(hid) {
        ...
        if (!this.modelfound) {
            ...
            warningElem.setAttribute('uk-tooltip', 'title: ' + SWAC_language.Worldmap.no_model);
