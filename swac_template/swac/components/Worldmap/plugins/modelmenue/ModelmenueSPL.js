/* 
 * Modelmenue plugin for this.requestor.swac_comp. Provides a menue for 3D modules with
 * enhanced navigation possibilites.
 */
var modelmenueFactory = {};
modelmenueFactory.create = function (pluginconfig) {
    return new ModelmenueSPL(pluginconfig);
};

class ModelmenueSPL extends ComponentPlugin {

    constructor(pluginconf) {
        super(pluginconf);
        this.componentName = 'cesium/plugins/modelmenue';
        this.desc = {};
        this.desc.depends = [];
        this.desc.templates = [];
    ...
    init() {
        ...
        //TODO what does this?
        this.requestor.swac_comp.options.onGoToModelFunctions.push(this.show.bind(this));
        // Register handler for camera moves
        this.requestor.swac_comp.viewer.camera.changed.addEventListener(this.onCameraChanged.bind(this));

        // Register event handler for picking
        var handler = new Cesium.ScreenSpaceEventHandler(this.requestor.swac_comp.viewer.scene.canvas);
        ...
        window.addEventListener("resize", function (evt) {
            ...
            for (let curRequestor of cesiumRequestors) {
                ...
                let curPlug = curRequestor.swac_comp.plugins.plugins.get('modelmenue');
    ...
    onCameraChanged(unkonwnparameter) {
        ...
        this.requestor.swac_comp.view.heading = Math.round(Cesium.Math.toDegrees(this.requestor.swac_comp.viewer.camera.heading));
        // Update pitch information to the current value of the cam
        this.requestor.swac_comp.view.pitch = Math.round(Cesium.Math.toDegrees(this.requestor.swac_comp.viewer.camera.pitch));
        // Update menue
//        this.update();
    }
...
//        // Get modelmenue plugin
//        let thisPlug = requestor.swac_comp.plugins.plugins.get('modelmenue');

        var pick = this.requestor.swac_comp.viewer.scene.pick(movement.position);
    ...
    onModelPicked(pick) {
        ...
        if (!pickedSwacModel && pick.id && pick.id.groundmodel) {
            ...
            SWAC_debug.addDebugMessage('ModelmenueSPL', 'Could not determine picked model.');
        }
        ...
        if (!navElem.model || pickedSwacModel !== navElem.model) {
            this.requestor.swac_comp.cesiumnavigation.gotoModel(pickedSwacModel);
    ...
    show(model, loc) {
        ...
        thisPlug.positionMenueElements(this.requestor);

        this.requestor.swac_comp.view.heading = 0;
        thisPlug.moveCam();
...
//    console.log('Pitch:', deg);
        ...
        if ((this.requestor.swac_comp.view.heading >= 337.5 && this.requestor.swac_comp.view.heading <= 360)
                || (this.requestor.swac_comp.view.heading >= 0 && this.requestor.swac_comp.view.heading < 22.5)) {
            selectElem.value = 0;
        } else if (this.requestor.swac_comp.view.heading >= 22.5 && this.requestor.swac_comp.view.heading < 67.5) {

        } else if (this.requestor.swac_comp.view.heading >= 67.5 && this.requestor.swac_comp.view.heading < 112.5) {
            selectElem.value = 90;
        } else if (this.requestor.swac_comp.view.heading >= 112.5 && this.requestor.swac_comp.view.heading < 157.5) {

        } else if (this.requestor.swac_comp.view.heading >= 157.5 && this.requestor.swac_comp.view.heading < 202.5) {
            selectElem.value = 180;
        } else if (this.requestor.swac_comp.view.heading >= 202.5 && this.requestor.swac_comp.view.heading < 247.5) {

        } else if (this.requestor.swac_comp.view.heading >= 247.5 && this.requestor.swac_comp.view.heading < 292.5) {
            selectElem.value = 270;
        } else if (this.requestor.swac_comp.view.heading >= 292.5 && this.requestor.swac_comp.view.heading < 337.5) {
    ...
    hide(evt) {
        ...
        var canvas = this.requestor.swac_comp.viewer.canvas;
...
// Reset view, so that users can move over map instead of rotating arount an point
        this.requestor.swac_comp.viewer.camera.lookAtTransform(Cesium.Matrix4.IDENTITY);
    }
    ...
    onClickAddToFavs(evt) {
        ...
        if (this.requestor.swac_comp.view.heading !== 0) {
            url = url + '&heading=' + this.requestor.swac_comp.view.heading;
        }
        // Add current pith to fav link
        if (this.requestor.swac_comp.view.pitch !== 0.0) {
            url = url + '&pitch=' + this.requestor.swac_comp.view.pitch;
        }

        // Remove hash
        url = url.replace('#', '');
        let infotxt = SWAC_language.cesium.modelmenue.favlink.replace(new RegExp('%url%', 'g'), url);
    ...
    onClickShowDetail(evt) {
        ...
        this.requestor.swac_view.insertLoadingElem(menueArea, 'Bitte einen Moment Geduld. Detailmodell wird geladen.');
        ...
        modelFactory.loadModel(hdurl, thisPlug.requestor, thisPlug.options.onShowDetail).then(function (model) {
            this.requestor.swac_comp.models.push(model);
            // Draw model
            model.draw(this.requestor.swac_comp.viewer).then(function (entityref) {
                this.requestor.swac_comp.cesiumnavigation.gotoModel(model, loc);
                console.log(entityref);
                // Remove loading element
                this.requestor.swac_view.removeLoadingElem(menueArea);
            });
        });
    }
    ...
    startMoveAround(evt) {
        ...
        moveButton.setAttribute('uk-tooltip', 'title: ' + SWAC_language.cesium.modelmenue.stopanimation);
    ...
    stopMoveAround(evt) {
        ...
        moveButton.setAttribute('uk-tooltip', 'title: ' + SWAC_language.cesium.modelmenue.startanimation);
    ...
    moveLittleStepRight() {
        ...
        if (this.requestor.swac_comp.view.heading < 0) {
            this.requestor.swac_comp.view.heading = 360;
        }
        if (isNaN(thisPlug.options.annimationstepsize)) {
            SWAC_debug.addErrorMessage('ModelmenueSPL', 'Option >annimationstepsize< is not a number. Useing stepsize 2');
            this.requestor.swac_comp.view.heading = this.requestor.swac_comp.view.heading - 2;
        } else {
            this.requestor.swac_comp.view.heading = this.requestor.swac_comp.view.heading - thisPlug.options.annimationstepsize;
        }
        thisPlug.moveCam();
    }
    ...
    moveToStartView(evt) {
        ...
        this.requestor.swac_comp.view.heading = 0.0;
        this.requestor.swac_comp.view.pitch = -33.0;
        thisPlug.moveCam();
    }
    ...
    moveStepRight(evt) {
        ...
        if (this.requestor.swac_comp.view.heading < 90) {
            this.requestor.swac_comp.view.heading = 360;
        }
        this.requestor.swac_comp.view.heading = this.requestor.swac_comp.view.heading - 90;
        thisPlug.moveCam();
    }
    ...
    moveStepLeft(evt) {
        ...
        if (this.requestor.swac_comp.view.heading >= 360) {
            this.requestor.swac_comp.view.heading = 0;
        }
        this.requestor.swac_comp.view.heading = this.requestor.swac_comp.view.heading + 90;
        thisPlug.moveCam();
    ...
    moveToHeading(evt) {
        ...
        this.requestor.swac_comp.view.heading = evt.target.value;
        thisPlug.moveCam();
    }
    ...
    moveCam() {
        ...
        if (!navElem.forModel.locations) {
            SWAC_debug.addErrorMessage('ModelmenueSPL', 'Model has no location. Cant move to model.');
            return;
        }
        if (!navElem.forModel.locations[loc]) {
            SWAC_debug.addDebugMessage('ModelmenueSPL', 'Model has no location >' + loc + '<. Cant move to model.');
            return;
        }
        let lon;
        let lat;
        ...
        let heading = Cesium.Math.toRadians(this.requestor.swac_comp.view.heading);
        let pitch = Cesium.Math.toRadians(this.requestor.swac_comp.view.pitch);
        let distance = this.requestor.swac_comp.view.optdistances[navElem.forModel.file.url];

        this.requestor.swac_comp.viewer.camera.lookAt(center, new Cesium.HeadingPitchRange(heading, pitch, distance));
