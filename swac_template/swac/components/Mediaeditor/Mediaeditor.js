...
class Mediaeditor extends Component {
    ...
    constructor(options = {}) {
        super(options);
        this.componentName = 'Mediaeditor';
...
//        });
    ...
    createMediaEditor(fromName, set) {
        ...
        if (set.mimetype.indexOf('image/') !== -1) {
            ...
            SWAC_debug.addErrorMessage('Mediaeditor', "Mimetype >" + set.mimetype + "< is not supported.");
        ...
        editorPromise.then(function (editorElem) {
            ...
            if (!mediaContentAreaFound) {
                SWAC_debug.addErrorMessage('Mediaeditor', 'Could not find element with class >swac_mediacontent<');
            }
        }).catch(function (error) {
            SWAC_debug.addErrorMessage('Mediaeditor', 'Could not create editor for >' + fromName + '/' + set.id + '<: ' + error);
        });
    }
    ...
    createImageEditor(fromName, set) {
        return new Promise((resolve, reject) => {
            SWAC_debug.addDebugMessage('Mediaeditor', 'Create imageEditor for >' + set.id + '<');
            let img = new Image();
            img.onload = function () {
...
//*************************************
    ...
    getMediaContArea(fromName, setid) {
        ...
        if (mediaAreas.length < 1) {
            SWAC_debug.addErrorMessage('Mediaeditor', 'Could not find mediaArea for >setname=' + fromName + ', setid=' + setid + '<');
            return null;
        }
