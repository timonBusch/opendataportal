 ...
 */
var mediaanalysisFactory = {};
mediaanalysisFactory.create = function (pluginconfig) {
    return new MediaanalysisSPL(pluginconfig);
};

class MediaanalysisSPL extends ComponentPlugin {

    constructor(pluginconf) {
        super(pluginconf);
        this.componentName = 'Mediaeditor/plugins/mediaanalysis';
...
for the id of the media. If id is send the picture itself will not be send.'
    ...
    init() {
        ...
        if (!this.requestor.swac_comp.plugins.loadedPlugins.includes('mediatags')) {
            SWAC_debug.addErrorMessage('MediaanalysisSPL', 'This plugin requires the mediatags plugin loaded too.', this.requestor);
            return;
        }
        if (!this.options.analysisrequestor) {
            SWAC_debug.addErrorMessage('MediaanalysisSPL', 'The option >analysisrequestor< is not set, cant run a analysis.', this.requestor);
            return;
        }

        // Register analyse function
        let startButtons = this.requestor.querySelectorAll(".swac_mediaanalysis_start");
    ...
    startAnalysis(evt) {
        ...
        let setElem = this.requestor.swac_view.findReapeatedForSet(evt.target);
        ...
        // Get mediatagsSPL
        let mediatagsSPL = this.requestor.swac_comp.plugins.plugins.get('mediatags');

        // Execute analysis
        let thisRef = this;
        SWAC_model.load(requestor).then(
                function (response) {
                    ...
                    for (let i = 0; i < tags.length; i++) {
                        ...
                        if (!polygons) {
                            SWAC_debug.addErrorMessage('MediaanalysisSPL', 'The >polygons< attribute is missing in the analysis result >' + i + '<', thisRef.requestor);
                        } else if (typeof polygons === 'string') {
                            SWAC_debug.addDebugMessage('MediaanalysisSPL', 'The >polygons< attribute in result >' + i + '< has string format. Please consider to change the used interface to deliver json.', thisRef.requestor);
                ...
                function (error) {
                    SWAC_debug.addErrorMessage('MediaanalysisSPL', 'Error analyising the media: ' + error);
                    UIkit.notification({
                        message: SWAC_language.Mediaeditor.mediaanalysis.analysiserror,
                        status: 'info',
