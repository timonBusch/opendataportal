/* 
 * Meadiareference plugin for SWAC_mediaeditor for creating references between 
 * medias.
 */
var mediareferenceFactory = {};
mediareferenceFactory.create = function (pluginconfig) {
    return new MediareferenceSPL(pluginconfig);
};

class MediareferneceSPL extends ComponentPlugin {

    constructor(pluginconf) {
        super(pluginconf);
        this.componentName = 'mediaeditor/plugins/mediareference';
        this.desc.templates[0] = {
            name: 'mediareference',
            style: 'mediareference',
            desc: 'Default template createing gui elements for createing media referneces'
        };
     ...
     * @returns {undefined}
     */
    createListElemFromJoiner(joiner, pluginarea) {
     ...
     */
    switchToSelectOneMediaMode(evt) {
    ...
    performCreateLink(evt) {
        ...
        remoteHandler.fetchPost("mediajoinmedia/create", querydata, false).then(
                function (joiner) {
     ...
     * @returns {undefined}
     */
    addAlphaOptionsToLayer(canvaslayer) {
